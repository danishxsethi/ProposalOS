...
async function generateExecutiveOverview(model: any, input: NarrativeInput): Promise<string> {
  ...
  const findingsSummary = painkiller
    .slice(0, 5)
    .map((f) => `- ${f.title}: ${f.description} (Impact: ${f.impactScore}/10)`)
    .join('\n');
...
async function generateClusterDeepDives(
  ...
  for (const [clusterName, findings] of Object.entries(clusters)) {
    ...
    const findingsText = painkillers
      .map((f) => `- ${f.title}: ${f.description} (Impact: ${f.impactScore}/10)`)
      .join('\n');
...
async function generateOpportunitySummary(model: any, input: NarrativeInput): Promise<string> {
  ...
  const prioritizedFindings = input.findings
    ...
    .map(
      (f, i) =>
        `Priority ${i + 1}: ${f.title} (Impact: ${f.impactScore}/10, Effort: ${f.effortEstimate})`
    )
    .join('\n');
...
function summarizeCompetitorData(competitorData: any): string {
  ...
  return competitorData.competitors
    .map((c: any) => `${c.name}: ${c.reviewCount || 0} reviews, ${c.rating || 'N/A'} stars`)
    .join('\n');
}
...
function validateQuality(text: string): QualityMetrics {
  ...
  const fleschKincaidGrade =
    0.39 * avgSentenceLength + 11.8 * (syllableCount / words.length) - 15.59;
