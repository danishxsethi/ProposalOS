...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  if (typeof seconds === 'number' && seconds >= 0) {
    leadUpdate.scorecardTotalViewSeconds = Math.max(
      lead.scorecardTotalViewSeconds,
      Math.round(seconds)
    );
  }
  ...
  if (event === 'view') {
    ...
    if (
      lead.outreachStage !== OutreachLeadStage.REPLIED &&
      lead.outreachStage !== OutreachLeadStage.DROPPED
    ) {
