...
export async function generateExecutiveSummary(
  ...
  const keyMetrics = findings
    .filter(
      (f) =>
        f.metrics && typeof f.metrics === 'object' && Object.keys(f.metrics as object).length > 0
    )
    .slice(0, 8)
    .map((f) => ({
      title: f.title,
  ...
  for (const m of keyMetrics) {
    ...
    for (const [k, v] of Object.entries(metrics || {})) {
      if (typeof v === 'number') numbersToCite.push(`${k}: ${v}`);
        ...
        numbersToCite.push(`${k}: ${(v as { value: unknown }).value}`);
    }
  }
  // Mine numbers from descriptions when structured metrics are sparse (e.g. "34/100", "4.2 seconds", "7 reviews")
  const descNumPattern =
    /\d+(?:\.\d+)?(?:\s*[\/%★]\d*|\s*(?:seconds?|ms|scores?|reviews?|points?|rating|percent|mb|kb))/gi;
  ...
  const winCount =
    comparisonReport?.summaryRow?.match(/winning on (\d+)/)?.[1] ??
    comparisonReport?.winningCategories?.length ??
    0;
  const loseCount =
    comparisonReport?.summaryRow?.match(/lead on (\d+)/)?.[1] ??
  ...
  return traceLlmCall(
    {
      ...
      inputs: {
        ...
        topIssues: clusterSummaries.slice(0, 3),
    ...
    async () => {
      try {
        ...
        text = text
          .replace(/\s+in\s+a\s+Key\s*metrics?\s*\.?$/i, '.')
        ...
        if (metricMatches.length < 2) {
          ...
          const m =
            numericValues[1] ?? (painkillers || Math.max(1, Math.min(findings.length - 1, 3)));
