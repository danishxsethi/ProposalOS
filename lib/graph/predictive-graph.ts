...
export const PredictiveState = Annotation.Root({
  ...
  industryBenchmarks: Annotation<Record<string, any>>({
    reducer: (x, y) => y,
    default: () => ({}),
  }),
...
async function forecastTraffic(state: typeof PredictiveState.State) {
  try {
    ...
    if (auditCount >= 2) {
      ...
      const scores = state.historicalAudits.map((a) => a.overallScore ?? 50).filter((s) => s > 0);
      const avgScoreGain =
        scores.length >= 2 ? (scores[0] - scores[scores.length - 1]) / scores.length : 0;
...
async function rankingTrajectoryNode(state: typeof PredictiveState.State) {
  try {
    const criticalFindings = state.currentFindings.filter((f) => (f.impactScore ?? 0) >= 7);
...
async function revenueImpactNode(state: typeof PredictiveState.State) {
  try {
    ...
    const upliftFromRanking =
      (state.rankingTrajectory?.withFixes.mid ?? 50) /
        (state.rankingTrajectory?.withoutFixes.mid ?? 45) -
      1;
