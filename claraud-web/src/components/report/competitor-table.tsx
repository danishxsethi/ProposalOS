    ...
    <div className="w-full">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="py-5 px-6 text-xs font-bold uppercase tracking-widest text-text-secondary">
                  Metric
                </th>
                ...
                {competitors.map((comp) => (
                  <th
                    key={comp.name}
                    className="py-5 px-6 text-xs font-bold uppercase tracking-widest text-text-secondary"
                  >
                    {comp.name}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-white/5 font-mono text-sm">
              {metrics.map((metric) => (
                <tr key={metric.label} className="group hover:bg-white/[0.02] transition-colors">
                  <td className="py-5 px-6 text-text-primary font-sans font-medium">
                    {metric.label}
                  </td>
                  ...
                  {competitors.map((comp) => {
                    ...
                    return (
                      <td
                        key={comp.name}
                        className={`py-5 px-6 font-medium ${getComparisonColor(userVal, compVal)}`}
                      >
              ...
              <tr className="bg-white/5">
                <td className="py-5 px-6 text-text-secondary font-sans text-xs italic">
                  Market Standing
                </td>
                ...
                {competitors.map((comp) => (
                  <td key={comp.name} className="py-5 px-6">
                    <span
                      className={`text-xs font-bold uppercase tracking-tighter ${comp.overallScore > userMetrics.overallScore ? 'text-green-500' : 'text-text-secondary'}`}
                    >
      ...
      <p className="mt-4 text-xs text-text-secondary flex items-center justify-center gap-4">
        <span className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-green-500/40" /> Winning
        </span>
        <span className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-red-500/40" /> Opp. identified
        </span>
        <span className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-yellow-500/40" /> Competitive parity
        </span>
      </p>
    </div>
  );
