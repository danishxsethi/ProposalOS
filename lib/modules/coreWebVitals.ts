 ...
 */
export function extractCoreWebVitalsFromAudits(
  audits: Record<
    string,
    {
      numericValue?: number;
  ...
  const resSummary = audits['resource-summary']?.details as
  ...
  const networkRequests = audits['network-requests']?.details as
    | { items?: Array<{ url?: string; transferSize?: number; resourceSize?: number }> }
    | undefined;
  ...
  return {
    ...
    tbt: buildMetric(
      tbtMs,
      CWV_THRESHOLDS.tbt.good,
      CWV_THRESHOLDS.tbt.poor,
      rateTbt,
      'milliseconds'
    ),
    ttfb: buildMetric(
      ttfbMs,
      CWV_THRESHOLDS.ttfb.good,
      CWV_THRESHOLDS.ttfb.poor,
      rateTtfb,
      'seconds'
    ),
    speedIndex: buildMetric(
      siMs,
      CWV_THRESHOLDS.speedIndex.good,
      CWV_THRESHOLDS.speedIndex.poor,
      rateSpeedIndex,
      'seconds'
    ),
    totalPageWeightMB,
