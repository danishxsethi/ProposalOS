...
const ProposalChatWidget = dynamic(
  () => import('@/components/chat/ProposalChatWidget').then((mod) => mod.ProposalChatWidget),
  {
    ssr: false,
    loading: () => null,
  }
...
function AnimatedGauge({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`${label} Score is ${Math.round(display)} out of 100`}
    >
...
export default function ProposalPage({ proposal, branding }: ProposalProps) {
  ...
  const visualEvidenceMap = new Map<
    string,
    { screenshotUrl: string; annotationText: string; findingRef: string; severity: number }
  >();
  ...
  const matrixData = (() => {
    ...
    const f = findings.find((x: { evidence?: unknown[] }) =>
      (x.evidence as Array<{ matrix?: unknown; raw?: { matrix?: unknown } }>)?.some?.(
        (e) => e.matrix || e.raw?.matrix
      )
    );
    const firstEvidence = (
      f?.evidence as Array<{
        matrix?: { business: MatrixRow; competitors: MatrixRow[] };
        raw?: { matrix?: { business: MatrixRow; competitors: MatrixRow[] } };
      }>
    )?.[0];
  ...
  const handleCheckout = async (tierId: string) => {
    ...
    try {
      const res = await fetch('/api/stripe/checkout-proposal', {
        method: 'POST',
  ...
  const submitContact = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/proposal/token/${proposal.webLinkToken}/contact`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <main className="min-h-screen text-white" style={{ backgroundColor: NAVY }}>
              ...
              <form onSubmit={submitContact} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-[#4361ee] min-h-[44px]"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    value={contactForm.preferredTier}
                    onChange={(e) =>
                      setContactForm({ ...contactForm, preferredTier: e.target.value })
                    }
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[#4361ee] min-h-[44px]"
                  >
                    <option value="">Select...</option>
                    <option value="starter" className="bg-[#16213e]">
                      Starter
                    </option>
                    <option value="growth" className="bg-[#16213e]">
                      Growth
                    </option>
                    <option value="premium" className="bg-[#16213e]">
                      Premium
                    </option>
                  </select>
                </div>
                <div>
                  ...
                  <select
                    value={contactForm.bestTime}
                    onChange={(e) => setContactForm({ ...contactForm, bestTime: e.target.value })}
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[#4361ee] min-h-[44px]"
                  >
                    <option value="">Select...</option>
                    {BEST_TIME_OPTIONS.map((o) => (
                      <option key={o} value={o} className="bg-[#16213e]">
                        {o}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-[#4361ee] resize-none"
                  />
                </div>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full py-4 rounded-xl font-bold text-white transition-opacity disabled:opacity-50 min-h-[44px]"
                  style={{ backgroundColor: ACCENT }}
                >
      ...
      <div className="sticky top-0 z-40 bg-[#1a1a2e]/95 backdrop-blur-md">
        {!urgencyBannerDismissed && (
          <div className="flex items-center justify-between gap-4 px-4 py-3 bg-amber-500/15 border-b border-amber-500/30 text-amber-100/95 text-sm">
            <p>
              This audit reflects <strong>{proposal.audit.businessName}</strong>&apos;s website as
              of{' '}
              {formatDate(
                (proposal.audit as { completedAt?: Date })?.completedAt || proposal.createdAt
              )}
              . Findings may change as competitors update their sites.
            </p>
            <button
              onClick={() => setUrgencyBannerDismissed(true)}
              className="flex-shrink-0 p-1 rounded hover:bg-amber-500/20 min-w-[44px] min-h-[44px] flex items-center justify-center"
              aria-label="Dismiss"
            >
              ×
            </button>
        ...
        <header className="border-b border-white/10">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <a
                href={`/api/proposal/token/${proposal.webLinkToken}/pdf`}
                onClick={() => trackCta('download_pdf')}
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2 rounded-lg text-sm font-medium text-white flex items-center gap-2 min-h-[44px] min-w-[44px] justify-center"
        ...
        <nav className="border-b border-white/5 py-3 px-4">
          <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:flex-wrap sm:justify-center gap-2 sm:gap-4 text-sm">
            <button
              ...
              className="sm:hidden flex items-center gap-2 text-white/70 hover:text-white min-h-[44px] px-2"
              ...
              <div className="flex flex-wrap gap-2 sm:hidden">
                {[
                  ...
                  <a
                    ...
                    className="text-white/70 hover:text-white hover:underline px-2 py-1 rounded min-h-[44px] flex items-center"
                  >
                    {id.replace(/-/g, ' ')}
                  </a>
                ))}
      ...
      <section
        ref={heroRef.ref}
        id="hero"
        className={`py-16 sm:py-24 px-4 sm:px-6 ${sectionClass(heroRef)}`}
      >
        <div className="max-w-5xl mx-auto text-center">
          ...
          {(proposal.audit.businessCity || proposal.audit.businessIndustry) && (
            <p className="text-white/50 text-sm mt-4">
              {proposal.audit.businessCity}
              {proposal.audit.businessCity && proposal.audit.businessIndustry && ' • '}
              {proposal.audit.businessIndustry}
            </p>
          )}
        </div>
      </section>
      ...
      {proposal.executiveSummary && (
        <section ref={execRef.ref} id="executive-summary" className={sectionClass(execRef)}>
          <div className="max-w-5xl mx-auto">
            <h2 className="text-2xl sm:text-3xl font-bold mb-6" style={{ color: BLUE }}>
              Executive Summary
            </h2>
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full px-4 sm:px-6 py-4 flex items-center justify-between gap-4 text-left min-h-[44px]"
                  >
                    ...
                    <svg
                      className={`w-5 h-5 flex-shrink-0 text-white/60 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      ...
                      {Array.isArray(f.recommendedFix) && f.recommendedFix.length > 0 && (
                        <div className="mt-4">
                          ...
                          <ul className="space-y-1">
                            {(f.recommendedFix as string[]).map((fix, i) => (
      ...
      {(matrixData || comparisonReport) && (
        <section ref={competitorRef.ref} id="competitors" className={sectionClass(competitorRef)}>
          <div className="max-w-5xl mx-auto space-y-8">
            ...
            {comparisonReport && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-6">
                <p className="text-lg font-semibold text-white mb-2">
                  You rank #{comparisonReport.prospectRank} out of{' '}
                  {comparisonReport.competitors.length + 1}{' '}
                  {proposal.audit.businessIndustry || 'business'}s in your area
                </p>
                <p className="text-white/80">{comparisonReport.summaryStatement}</p>
                {comparisonReport.summaryRow && (
                  <p className="text-white font-semibold mt-2">{comparisonReport.summaryRow}</p>
                )}
              </div>
            )}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        ...
                        >
                          {
                            (comparisonReport?.prospect?.name ||
                              matrixData?.business?.name ||
                              proposal.audit.businessName) as string
                          }{' '}
                          (You)
                        </th>
                        {(
                          comparisonReport?.comparisonTableRows?.[0]?.competitorValues?.map(
                            (c) => c.name
                          ) ??
                          matrixData?.competitors
                            ?.slice(0, 3)
                            .map((c: { name?: string }) => c.name) ??
                          []
                        ).map((name: string | undefined, i: number) => (
                          <th
                            key={i}
                            className="p-4 text-center text-white/70 font-medium border-l border-white/10"
                          >
                            {name || `Competitor ${i + 1}`}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-white/5">
                      {comparisonReport?.comparisonTableRows?.map((row, i) => {
                        ...
                        return (
                          <tr key={i} className="group">
                            <td className="p-4 text-white/70" title={row.metric}>
                              {row.metric}
                            </td>
                            <td
                              className="p-4 text-center font-bold border-l border-white/5 transition-colors group-hover:bg-white/5"
                              style={{
                                backgroundColor: `${BLUE}10`,
                                ...(cellBg(row.prospectStatus)
                                  ? { backgroundColor: cellBg(row.prospectStatus) }
                                  : {}),
                              }}
                              title={`You: ${row.prospectValue}`}
                            >
                              {row.prospectValue}
                            </td>
                            {row.competitorValues.map((cv, j) => (
                              <td
                                key={j}
                                className="p-4 text-center text-white/60 border-l border-white/5 transition-colors hover:bg-white/5"
                                style={{ backgroundColor: cellBg(cv.status) }}
                                title={`${cv.name}: ${cv.value}`}
                              >
                                {cv.value}
                              </td>
                            ))}
                      ...
                      {!comparisonReport?.comparisonTableRows?.length && matrixData && (
                        <>
                          <tr>
                            <td className="p-4 text-white/70">PageSpeed</td>
                            <td
                              className="p-4 text-center font-bold"
                              style={{ backgroundColor: `${BLUE}10` }}
                            >
                              {formatScore(
                                matrixData.business.performanceScore ??
                                  matrixData.business.websiteSpeed
                              )}
                            </td>
                            {matrixData.competitors
                              .slice(0, 3)
                              .map(
                                (
                                  c: { performanceScore?: number; websiteSpeed?: number },
                                  i: number
                                ) => (
                                  <td
                                    key={i}
                                    className="p-4 text-center text-white/60 border-l border-white/5"
                                  >
                                    {formatScore(c.performanceScore ?? c.websiteSpeed)}
                                  </td>
                                )
                              )}
                          </tr>
                          <tr>
                            <td className="p-4 text-white/70">Mobile Score</td>
                            <td
                              className="p-4 text-center font-bold"
                              style={{ backgroundColor: `${BLUE}10` }}
                            >
                              {formatScore(
                                matrixData.business.mobileScore ??
                                  matrixData.business.performanceScore
                              )}
                            </td>
                            {matrixData.competitors
                              ...
                              .map(
                                (
                                  c: { mobileScore?: number; performanceScore?: number },
                                  i: number
                                ) => (
                                  <td
                                    key={i}
                                    className="p-4 text-center text-white/60 border-l border-white/5"
                                  >
                                    {formatScore(c.mobileScore ?? c.performanceScore)}
                                  </td>
                                )
                              )}
                          </tr>
                          <tr>
                            <td className="p-4 text-white/70">Load Time</td>
                            <td
                              className="p-4 text-center font-bold"
                              style={{ backgroundColor: `${BLUE}10` }}
                            >
                              {matrixData.business.loadTimeSeconds != null
                                ? `${matrixData.business.loadTimeSeconds}s`
                                : '—'}
                            </td>
                            {matrixData.competitors
                              .slice(0, 3)
                              .map((c: { loadTimeSeconds?: number }, i: number) => (
                                <td
                                  key={i}
                                  className="p-4 text-center text-white/60 border-l border-white/5"
                                >
                                  {c.loadTimeSeconds != null ? `${c.loadTimeSeconds}s` : '—'}
                                </td>
                              ))}
                          </tr>
                          <tr>
                            <td className="p-4 text-white/70">Google Rating</td>
                            <td
                              className="p-4 text-center font-bold"
                              style={{ backgroundColor: `${BLUE}10` }}
                            >
                              {matrixData.business.rating ?? '—'}★
                            </td>
                            {matrixData.competitors
                              .slice(0, 3)
                              .map((c: { rating?: number }, i: number) => (
                                <td
                                  key={i}
                                  className="p-4 text-center text-white/60 border-l border-white/5"
                                >
                          ...
                          <tr>
                            <td className="p-4 text-white/70">Reviews</td>
                            <td
                              className="p-4 text-center font-bold"
                              style={{ backgroundColor: `${BLUE}10` }}
                            >
                              {matrixData.business.reviewCount ?? '—'}
                            </td>
                            {matrixData.competitors
                              .slice(0, 3)
                              .map((c: { reviewCount?: number }, i: number) => (
                                <td
                                  key={i}
                                  className="p-4 text-center text-white/60 border-l border-white/5"
                                >
            ...
            {comparisonReport &&
              (comparisonReport.whereAhead?.length ??
                comparisonReport.winningCategories?.length ??
                0) > 0 && (
                <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-6">
                  ...
                  {(comparisonReport.whereAhead?.length ?? 0) > 0 ? (
                    <ul className="mt-2 space-y-2 text-white/90">
                      {comparisonReport.whereAhead!.map((s, i) => (
                    ...
                    <>
                      ...
                      <ul className="mt-2 space-y-1 text-white/80">
                        {comparisonReport.winningCategories.map((cat, i) => (
                          <li key={i} className="flex items-center gap-2">
                            <span className="text-green-400">✓</span>
                            {cat}
                          </li>
            ...
            {comparisonReport &&
              (comparisonReport.whereBehind?.length ??
                comparisonReport.losingCategories?.length ??
                0) > 0 && (
                <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-6">
                  ...
                  {(comparisonReport.whereBehind?.length ?? 0) > 0 ? (
                    <ul className="mt-2 space-y-2 text-white/90">
                      {comparisonReport.whereBehind!.map((s, i) => (
                        <li key={i} className="flex items-center gap-2">
                    ...
                    <>
                      ...
                      <ul className="mt-2 space-y-1 text-white/80">
                        {comparisonReport.losingCategories.map((cat, i) => (
                          <li key={i} className="flex items-center gap-2">
                            <span className="text-amber-400">→</span>
                            {cat}
                          </li>
                  ...
                  {comparisonReport.quickWins.map((qw, i) => (
                    <li key={i} className="flex flex-col gap-1">
                      ...
                      <span className="text-white/60 text-sm">
                        Effort: {qw.effortEstimate} • {qw.expectedImpact}
                      </span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            {tiers.map((t) => (
              <div
                key={t.id}
                className={`rounded-2xl p-6 sm:p-8 border transition-all ${t.recommended ? 'border-[#4361ee] shadow-lg scale-[1.02] md:scale-105' : 'border-white/10 bg-white/5'}`}
              >
                {(t.badge || (t.recommended && !t.badge)) && (
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl font-bold text-white min-h-[44px]"
                  style={{ backgroundColor: t.recommended ? ACCENT : 'rgba(255,255,255,0.15)' }}
                >
                  Get Started
                </button>
              </div>
        ...
        <section ref={nextStepsRef.ref} id="next-steps" className={sectionClass(nextStepsRef)}>
          <div className="max-w-5xl mx-auto">
            ...
            <ol className="list-decimal list-inside space-y-3 text-white/80">
              {proposal.nextSteps.map((s: string, i: number) => (
                <li key={i}>{s}</li>
              ))}
            </ol>
          </div>
      ...
      <section
        ref={ctaRef.ref}
        id="contact"
        className={`py-16 sm:py-24 px-4 sm:px-6 ${sectionClass(ctaRef)}`}
        style={{ background: `linear-gradient(180deg, transparent, ${BLUE}15)` }}
      >
        <div className="max-w-2xl mx-auto text-center">
          ...
          <button
            ...
            className="px-8 py-4 rounded-xl font-bold text-white text-lg min-h-[44px]"
            style={{ backgroundColor: ACCENT }}
          >
