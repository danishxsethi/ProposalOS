...
export default function ProposalPage({ proposal, branding }: ProposalProps) {
    ...
    const handleFeedback = async (findingId: string, score: number) => {
        setFeedbackMap(prev => ({ ...prev, [findingId]: score }));
        try {
            await fetch('/api/llm/feedback', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ runId: `finding-${findingId}`, score, comment: 'Feedback from Proposal Web Viewer' }),
            });
        } catch (e) {
                                ...
                                <div key={f.id} className="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
                                    ...
                                    {isExpanded && (
                                        <div className="px-4 sm:px-6 pb-4 border-t border-white/10">
                                            ...
                                            {Array.isArray(f.evidence) && (f.evidence as any[]).some(e => e.type === 'image' && (e.url || e.value)) && (
                                                <div className="mt-4 grid gap-4 grid-cols-1 sm:grid-cols-2">
                                                    {(f.evidence as any[]).filter(e => e.type === 'image' && (e.url || e.value)).map((img, i) => (
                                                        <div key={i} className="flex flex-col gap-2">
                                                            {/* eslint-disable-next-line @next/next/no-img-element */}
                                                            <img src={img.url || img.value} alt={img.label || 'Visual Evidence'} className="w-full rounded-lg border border-white/10" loading="lazy" />
                                                            {img.label && <span className="text-xs text-white/50">{img.label}</span>}
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                            ...
                                            <div className="mt-6 flex items-center gap-3 pt-4 border-t border-white/5">
                                                <span className="text-xs text-white/40">Was this finding completely accurate?</span>
                                                <div className="flex gap-1">
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleFeedback(f.id, 1); }}
                                                        className={`p-1.5 rounded transition-colors ${feedbackMap[f.id] === 1 ? 'bg-green-500/20 text-green-400' : 'hover:bg-white/10 text-white/40 hover:text-white/80'}`}
                                                    >
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" /></svg>
                                                    </button>
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleFeedback(f.id, 0); }}
                                                        className={`p-1.5 rounded transition-colors ${feedbackMap[f.id] === 0 ? 'bg-red-500/20 text-red-400' : 'hover:bg-white/10 text-white/40 hover:text-white/80'}`}
                                                    >
