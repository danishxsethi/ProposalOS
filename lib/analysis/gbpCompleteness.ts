...
function isDescriptionOptimal(d: GbpCompletenessInput): {
  ok: boolean;
  length: number;
  inRange: boolean;
} {
...
export function computeGbpCompleteness(
  ...
  breakdown.push({
    ...
    recommendation: !nameOk
      ? 'Add business name in GBP'
      : !addrOk
        ? 'Add full address (street, city, state/zip)'
        : !phoneOk
          ? 'Add phone number'
          : !websiteOk
            ? 'Link your website in GBP'
            : !nameMatch
              ? 'Ensure GBP name matches your website'
              : undefined,
  });
  ...
  breakdown.push({
    ...
    recommendation: !hoursOk
      ? 'Set hours for all 7 days in GBP — add special hours for holidays'
      : undefined,
  });
  ...
  const photoScore =
    photoCount >= 10
      ? WEIGHTS.photos
      : photoCount >= 5
        ? Math.round(WEIGHTS.photos * 0.6)
        : photoCount >= 1
          ? Math.round(WEIGHTS.photos * 0.2)
          : 0;
  breakdown.push({
    category: 'Photos',
    ...
    recommendation:
      photoCount < 10
        ? `Add ${10 - photoCount} more photos — businesses with 10+ photos get 35% more clicks`
        : undefined,
  });
  ...
  const respScore =
    respRate != null && respRate >= 0 ? (respRate >= 0.8 ? 4 : respRate > 0 ? 2 : 0) : 2; // Unknown = partial credit
  ...
  breakdown.push({
    ...
    recommendation:
      reviewCount < 10
        ? `Get more reviews — you have ${reviewCount}, aim for 20+`
        : rating < 4
          ? 'Improve rating — address negative feedback'
          : respRate === 0
            ? 'Respond to your reviews — 89% of consumers read business responses'
            : undefined,
  });
  ...
  breakdown.push({
    ...
    recommendation: !postsOk
      ? 'Start posting weekly Google updates — active profiles rank higher in local search'
      : undefined,
  });
  ...
  const descScore = descCheck.inRange
    ? WEIGHTS.description
    : descCheck.ok
      ? Math.round(WEIGHTS.description * 0.6)
      : 0;
  breakdown.push({
    ...
    recommendation: !descCheck.ok
      ? 'Add a business description (250-750 chars with keywords)'
      : !descCheck.inRange
        ? `Expand description to 250-750 chars (you have ${descCheck.length}) — include services and location keywords`
        : undefined,
  });
  ...
  const catScore = categoriesOptimal
    ? WEIGHTS.categories
    : hasPrimary
      ? Math.round(WEIGHTS.categories * 0.5)
      : 0;
  breakdown.push({
    category: 'Categories',
    ...
    recommendation: !hasPrimary
      ? 'Set primary category in GBP'
      : additionalCount < 2
        ? 'Add 2-5 additional categories to improve discoverability'
        : undefined,
  });
  totalScore += catScore;
  ...
  breakdown.push({
    ...
    recommendation: !servicesOk
      ? 'List services or products in GBP — include prices where possible'
      : undefined,
  });
  ...
  breakdown.push({
    ...
    weight: WEIGHTS.qa,
    recommendation: !qaOk
      ? 'Add Q&A and answer common questions — helps with "near me" searches'
      : undefined,
  });
  totalScore += qaScore;
  ...
  breakdown.push({
    ...
    recommendation: !attrsOk
      ? 'Set attributes (wheelchair accessible, free wifi, payment methods) in GBP'
      : undefined,
  });
  ...
  if (photoCount < 10 && photoCount > 0) {
    recommendations.push(
      `Add ${10 - photoCount} more photos of your office interior — businesses with 10+ photos get 35% more clicks`
    );
  } else if (photoCount === 0) {
    recommendations.push(
      'Upload 10+ photos (exterior, interior, team, products) — businesses with photos get 35% more clicks'
    );
  }
  if (reviewCount > 0 && respRate === 0) {
    recommendations.push(
      `Respond to your ${reviewCount} unanswered reviews — 89% of consumers read business responses`
    );
  }
  if (!postsOk) {
    recommendations.push(
  ...
  if (!descCheck.ok) {
    recommendations.push(
      'Add a 250-750 character description with keywords — helps with local SEO'
    );
  }
  if (!hoursOk) {
  ...
  if (!attrsOk) {
    recommendations.push(
      'Add attributes (wheelchair accessible, free wifi) — key filters in voice search'
    );
  }
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['No major improvements needed. Maintain current standards.'],
    competitorAvgScore,
