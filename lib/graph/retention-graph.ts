...
async function generate_summary(state: typeof RetentionState.State) {
  ...
  if (state.npsResults) {
    parts.push(
      `NPS: ${state.npsResults.sent} sent (Day30: ${state.npsResults.day30Processed}, Day90: ${state.npsResults.day90Processed})`
    );
  }

  if (state.competitorResults) {
    parts.push(
      `Competitor: ${state.competitorResults.signalsDetected} signals, ${state.competitorResults.upsellsTriggered} upsells triggered`
    );
  }

  if (state.scheduledAuditResults) {
    parts.push(
      `Scheduled Audits: ${state.scheduledAuditResults.auditsRun} run, ${state.scheduledAuditResults.comparisonsGenerated} comparisons`
    );
  }

  const summary = parts.length > 0 ? parts.join(' | ') : 'No retention actions needed';
...
export async function runRetentionWorkflow(): Promise<{
  ...
  const summary =
    [
      ...
      competitorResult.upsellsTriggered > 0
        ? `${competitorResult.upsellsTriggered} upsells triggered`
        : null,
      auditResult.auditsRun > 0 ? `${auditResult.auditsRun} scheduled audits run` : null,
    ]
      .filter(Boolean)
      .join(', ') || 'No retention actions needed';
...
export async function getClientRetentionStatus(proposalId: string) {
  ...
  const npsStatus = {
    day30Due:
      daysSinceAcceptance !== null && daysSinceAcceptance >= 30 && daysSinceAcceptance <= 31,
    day90Due:
      daysSinceAcceptance !== null && daysSinceAcceptance >= 90 && daysSinceAcceptance <= 91,
  ...
  return {
    ...
    emailSequence: proposal.emailSequence
      ? {
          emailsSent: (proposal.emailSequence.emails as any[]).filter(
            (e: any) => e.status === 'sent'
          ).length,
