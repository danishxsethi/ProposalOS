...
export function AuditRadarChart({ data }: RadarChartProps) {
  ...
  return (
    <motion.div
      ...
      role="img"
      aria-label={`Radar chart showing performance profile across ${data.length} categories. Average score: ${averageScore.toFixed(1)} out of 10. Categories: ${data.map(d => `${d.category}: ${d.score}`).join(', ')}`}
    >
