...
const ClosingStateAnnotation = Annotation.Root({
    proposalId: Annotation<string>({ reducer: (x, y) => y ?? x, default: () => "" }),
    sessionId: Annotation<string>({ reducer: (x, y) => y ?? x, default: () => "" }),
    businessName: Annotation<string>({ reducer: (x, y) => y ?? x, default: () => "" }),
    prospectContext: Annotation<string>({ reducer: (x, y) => y ?? x, default: () => "" }),
    messages: Annotation<BaseMessage[]>({ reducer: (x, y) => x.concat(y), default: () => [] }),
    lastSentiment: Annotation<number>({ reducer: (x, y) => y ?? x, default: () => 0 }),
    escalated: Annotation<boolean>({ reducer: (x, y) => y ?? x, default: () => false }),
    agentResponse: Annotation<string>({ reducer: (x, y) => y ?? x, default: () => "" }),
});

async function processInput(state: typeof ClosingStateAnnotation.State) {
...
async function generateResponse(state: typeof ClosingStateAnnotation.State) {
...
function routeAfterInput(state: typeof ClosingStateAnnotation.State) {
...
async function escalationHandler(state: typeof ClosingStateAnnotation.State) {
...
export const closingGraph = new StateGraph(ClosingStateAnnotation)
