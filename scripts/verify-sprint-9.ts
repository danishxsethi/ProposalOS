...
async function verifySprint9() {
  ...
  if (!proposals.length) {
    console.log(
      '⚠️ No completed proposals found in DB to test graph nodes. Please route an E2E audit first.'
    );
    return;
  }
  ...
  if (chatResult1.reply && chatResult1.sentiment < 0) {
    console.log(
      '✅ Agent correctly invoked objection template without hallucinating. Sentiment decayed:',
      chatResult1.sentiment
    );
  } else {
    throw new Error(
      '❌ Agent failed to respond to the price objection correctly or update tracked sentiment.'
    );
  }
  ...
  if (state && state.objectionsRaised.includes('price')) {
    console.log(
      '✅ Conversation Memory accurately tracked the sequence and objections within Postgres JSON.'
    );
  } else {
  ...
  if (emailRes.emails && emailRes.emails.length >= 1) {
    console.log(
      `✅ Email generator returned ${emailRes.emails.length}/5 emails (partial runs expected in context-limited test env).`
    );
