...
async function executeModuleWithRetry<T>(
    moduleName: string,
    moduleFn: () => Promise<T>,
    maxRetries = 2
): Promise<T> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // If the module explicitly returns a failed status, throw to trigger retry
            if (result && typeof result === 'object' && 'status' in result && result.status === 'failed') {
                throw new Error((result as any).error || `${moduleName} failed`);
            }
            return result;
        } catch (error) {
            lastError = error;
            logger.warn({ module: moduleName, attempt, maxRetries, error: String(error) }, 'Module execution failed, retrying...');
            if (attempt < maxRetries) {
                await new Promise(resolve => setTimeout(resolve, 1500));
            }
        }
    }
    // If all retries fail, returning a failed structured result prevents Promise.allSettled from rejecting the whole wrapper
    return { status: 'failed', error: lastError } as any;
}
...
export async function runAudit(auditId: string) {
    ...
    try {
        ...
        const [websiteResult, gbpResult, competitorResult] = await Promise.allSettled([
            url ? executeModuleWithRetry('website', () => runWebsiteModule({ url }, tracker)) : Promise.resolve({ status: 'failed', error: 'No URL provided' } as any),
            name && city ? executeModuleWithRetry('gbp', () => runGBPModule({ businessName: name, city, websiteUrl: url ?? undefined }, tracker)) : Promise.resolve({ status: 'failed', error: 'No name/city' } as any),
            name && city ? executeModuleWithRetry('competitor', () => runCompetitorModule({ keyword: name, location: city }, tracker)) : Promise.resolve({ status: 'failed', error: 'No name/city' } as any),
        ]);
        ...
        if (!modulesCompleted.includes('website')) {
            allFindings.push({
                module: 'website',
                category: 'system',
                type: 'DATA_UNAVAILABLE' as any,
                title: 'Website Analysis Unavailable',
                description: 'We were unable to successfully scan the website during this audit pass due to a timeout or security block. Ensure the URL is accessible without a captcha.',
                evidence: [{ type: 'text', value: 'Website scraper failed or timed out', label: 'Error' }],
                metrics: { available: false },
                impactScore: 0,
                confidenceScore: 0,
                effortEstimate: 'LOW',
                recommendedFix: ['Verify the website is online', 'Check for anti-bot protection']
            });
        }
        ...
        if (competitorResult.status === 'fulfilled' && competitorResult.value.status === 'success') {
            ...
            if (!modulesCompleted.includes('competitor')) {
                allFindings.push({
                    module: 'competitor',
                    category: 'system',
                    type: 'VITAMIN',
                    title: 'Competitor Analysis Unavailable',
                    description: 'Unable to retrieve competitor search rankings for this location at this time.',
                    evidence: [{ type: 'text', value: 'SerpAPI search failed or timed out', label: 'Error' }],
                    metrics: { available: false },
                    impactScore: 1,
                    confidenceScore: 100,
                    effortEstimate: 'LOW',
                    recommendedFix: ['Try running the audit again later']
                });
            }
        }
        ...
        if (gbpResult.status === 'fulfilled' && gbpResult.value.status === 'success' && gbpResult.value.data.reviews?.length > 0) {
            const reputationResult = await executeModuleWithRetry('reputation', () => runReputationModule(
        ...
        if (url) {
            const socialResult = await executeModuleWithRetry('social', () => runSocialModule(
        ...
        if (url) {
            const gbpTypes = gbpResult.status === 'fulfilled' && gbpResult.value.status === 'success'
                ...
                : undefined;
            const schemaMarkupResult = await executeModuleWithRetry('schema', () => runSchemaMarkupModule(
        ...
        if (url) {
            const accessibilityResult = await executeModuleWithRetry('accessibility', () => runAccessibilityModule({ url }, tracker));
        ...
        if (url) {
            const securityResult = await executeModuleWithRetry('security', () => runSecurityModule({ url }, tracker));
        ...
        if (url) {
            ...
            const conversionResult = await executeModuleWithRetry('conversion', () => runConversionModule({ url, businessName: name, industry: detectedIndustry }, tracker));
            ...
            // Run Screenshot Orchestration and Vision Pipeline
            try {
                const compUrls = competitorData?.topCompetitors
                    ?.map((c: any) => c.website || c.url)
                    .filter(Boolean) || [];

                const screenshots = await orchestrateScreenshots({
                    auditId: audit.id,
                    businessUrl: url,
                ...
                if (screenshots.allScreenshots.length > 0) {
                    const visionResult = await executeModuleWithRetry('vision', () => runVisionModule({
                        auditId: audit.id,
                        businessName: name || 'Unknown',
                        industry: detectedIndustry || 'general',
                        screenshots: screenshots.allScreenshots
                    }, tracker));

                    if (visionResult.findings?.length > 0) {
                        modulesCompleted.push('vision');
                        Metrics.increment('modules_total');
                        logger.info({
                            event: 'module.complete',
                            auditId: audit.id,
                        ...
                        allFindings.push(...visionResult.findings);

                        for (const snap of visionResult.evidenceSnapshots || []) {
                            await prisma.evidenceSnapshot.create({
                                data: {
                                    auditId: audit.id,
                                    module: snap.module || 'vision',
                                    source: snap.source || 'gemini_31_pro',
                                    rawResponse: snap.rawResponse ?? snap,
                                    tenantId: audit.tenantId,
                                },
                            });
                ...
                logger.error({ auditId: audit.id, error: visionError }, 'Vision module execution failed');
                modulesFailed.push({ module: 'vision', error: String(visionError) });
                Metrics.increment('modules_failed');
            }
        }

        // Run Adversarial QA Graph (Fix 3D)
        if (allFindings.length > 0) {
            try {
                // Dynamically import the QA graph to avoid circular dependencies if any
                const { adversarialQAGraph } = await import('@/lib/graph/adversarial-qa-graph');

                logger.info({ auditId: audit.id, count: allFindings.length }, 'Invoking Adversarial QA Graph');
                const qaResult = await adversarialQAGraph.invoke({
                    auditId: audit.id,
                    findings: allFindings,
                });

                if (qaResult?.findings?.length > 0) {
                    // Replace original findings with the refined/filtered findings
                    allFindings.length = 0;
                    allFindings.push(...qaResult.findings);

                    logger.info({
                        auditId: audit.id,
                        finalCount: allFindings.length
                    }, 'Adversarial QA Graph complete');
                }
            } catch (qaError) {
                // If the QA graph fails, log the error but proceed with the original findings
                // We don't want a QA failure to completely break the audit delivery
                logger.error({
                    auditId: audit.id,
                    error: qaError
                }, 'Adversarial QA Graph failed. Proceeding with un-filtered findings.');
            }
        }
