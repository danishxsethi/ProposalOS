...
export async function runSecurityModule(
  ...
  try {
    ...
    if (hsts) {
      ...
      const status =
        age >= 31536000 && hasIncludeSubdomains ? 'good' : age >= 86400 ? 'weak' : 'weak';
      headerChecks.push({
        ...
        recommendation:
          status === 'good'
            ...
            : 'Set max-age to at least 31536000 (1 year) and includeSubDomains.',
      });
      ...
      headerChecks.push({
        ...
        status: 'missing',
        recommendation:
          'Add HSTS header to prevent downgrade attacks. Example: Strict-Transport-Security: max-age=31536000; includeSubDomains',
      });
    }
    ...
    if (csp) {
      ...
      headerChecks.push({
        ...
        recommendation:
          status === 'good'
            ? 'CSP is present.'
            : 'Add default-src directive to restrict resource loading.',
      });
      score += status === 'good' ? headerPoints : headerPoints * 0.5;
    } else {
      headerChecks.push({
    ...
    if (perm) {
      ...
      headerChecks.push({
        ...
        status: 'missing',
        recommendation:
          'Add Permissions-Policy to restrict browser features (camera, geolocation, etc.).',
      });
    }
    ...
    if (!httpsEnabled) {
      recommendations.push(
        'Enable HTTPS. Visitors see a "Not Secure" warning when your site uses HTTP.'
      );
    ...
    if (!certificate.valid && httpsEnabled) {
      recommendations.push(
        'Fix your SSL certificate. An invalid or expired certificate causes browser warnings.'
      );
    } else if (certificate.valid && certificate.expiresAt) {
      const daysLeft = Math.floor(
        (new Date(certificate.expiresAt).getTime() - Date.now()) / 86400000
      );
      if (daysLeft < 30) {
        recommendations.push(
          `SSL certificate expires in ${daysLeft} days. Renew before it expires.`
        );
      }
    }

    headerChecks
      .filter((h) => h.status === 'missing' || h.status === 'weak')
      .forEach((h) => {
      ...
      data: {
        ...
        recommendations:
          recommendations.length > 0
            ? recommendations
            : ['Your site has strong security headers. Maintain these standards.'],
      },
    };
