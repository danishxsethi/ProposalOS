...
function detectCms($: cheerio.CheerioAPI, html: string, headers?: Headers): string[] {
  ...
  if (headers && Array.from(headers.keys()).some((k) => k.toLowerCase().startsWith('x-wix')))
    detected.add('Wix');
  ...
  // Shopify
  if (html.includes('cdn.shopify.com') || html.includes('Shopify.designMode'))
  ...
  // GoDaddy
  if (html.includes('godaddy.com') || html.includes('GoCentral'))
    detected.add('GoDaddy Website Builder');
...
function detectAnalytics($: cheerio.CheerioAPI, html: string): string[] {
  ...
  if (html.includes('google-analytics.com/analytics.js') || html.includes('gtag.js'))
...
function generateTechFindings(stack: TechStack): Finding[] {
  ...
  if (usedBuilder) {
    findings.push({
      ...
      recommendedFix: [
        'Consider migrating to WordPress for better SEO control',
        'Monitor page speed closely',
      ],
    });
  }
  ...
  const isModern =
    stack.frameworks.some((f) => ['React', 'Vue.js', 'Next.js'].includes(f)) ||
  ...
  if (stack.analytics.length > 0 && stack.marketing.length > 0 && stack.widgets.length > 0) {
    findings.push({
      ...
      description:
        'You have a solid suite of tools for analytics, marketing, and customer interaction. This indicates a high level of digital maturity.',
      impactScore: 2,
      confidenceScore: normalizeConfidence(90, '0-100'),
      evidence: [
