...
describe('Locale Detector - Property-Based Tests', () => {
  ...
  const hreflangHtmlArb = fc.oneof(
    ...SUPPORTED_LOCALES.map((locale) =>
      fc.constant(
        `<html><head><link rel="alternate" hreflang="${locale}" href="/" /></head></html>`
      )
    )
  );
  ...
  describe('Additional Property Tests for Robustness', () => {
        ...
        fc.asyncProperty(
          ...
          async (context: DetectionContext) => {
            ...
            expect([
              ...
              'manual_override',
              'default',
            ]).toContain(result.detectionMethod);
            expect(result.confidence).toBeGreaterThanOrEqual(0);
            expect(result.confidence).toBeLessThanOrEqual(1);
    ...
    it('should provide locale configuration for any detected locale', async () => {
      await fc.assert(
        fc.asyncProperty(supportedLocaleArb, async (locale: string) => {
