...
export class LocalizedPromptLibrary {
  ...
  async getStats(): Promise<PromptLibraryStats> {
    try {
      const [nodesResult, localesResult, pendingResult, approvedResult] = await Promise.all([
        ...
        query(`SELECT COUNT(*) AS total FROM localized_prompts WHERE approval_status = 'pending'`),
        query(`SELECT COUNT(*) AS total FROM localized_prompts WHERE approval_status = 'approved'`),
      ]);
  ...
  async validateCompleteness(locale: string): Promise<ValidationResult> {
    ...
    try {
      ...
      const approvedNodeIds = new Set<string>(approvedResult.rows.map((r: any) => r.node_id));
      ...
      for (const nodeId of allNodeIds) {
        if (!approvedNodeIds.has(nodeId)) {
          ...
          if (pendingResult.rows.length > 0) {
            ...
            errors.push(`Node '${nodeId}' is missing an approved variant for locale '${locale}'`);
          }
        }
      }
