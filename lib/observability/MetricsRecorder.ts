...
class MetricsRecorderSingleton {
  ...
  auditCompleted(tenantId: string, status: string, durationMs: number, costUsd: number): void {
    this.record('audit_duration_ms', durationMs, { tenantId, status });
    this.record('audit_cost_usd', costUsd, { tenantId, status });
    this.record('audit_failure_rate', status === 'FAILED' ? 1 : 0, { tenantId });
  }

  requestCompleted(route: string, method: string, statusCode: number, durationMs: number): void {
    this.record('api_requests_total', 1, { route, method, statusCode: String(statusCode) });
    this.record('api_request_duration_ms', durationMs, {
      route,
      method,
      statusCode: String(statusCode),
    });
    this.record('api_error_rate', statusCode >= 500 ? 1 : 0, { route, method });
  }

  proposalGenerated(tenantId: string, status: string, qualityScore?: number): void {
    this.record('proposals_total', 1, { tenantId, status });
    if (typeof qualityScore === 'number') {
      this.record('proposal_quality_score', qualityScore, { tenantId, status });
    }
  }

  emailBounceRate(rate: number, tenantId: string): void {
    this.record('email_bounce_rate', rate, { tenantId });
  }

  queueBacklog(depth: number, queueName: string): void {
    this.record('queue_backlog', depth, { queueName });
  }
