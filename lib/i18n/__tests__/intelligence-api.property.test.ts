...
import { AuditFinding, PatternDiscoveryEngine } from '../pattern-discovery-engine';
...
const pluginArb = fc.option(fc.constantFrom('Yoast', 'RankMath', 'AllInOneSEO'), {
  nil: undefined,
});
...
const requesterIdArb = fc
  .string({ minLength: 3, maxLength: 20 })
  .filter((s) => s.trim().length > 0);
const recommendationTypeArb = fc.constantFrom(
  'schema_markup',
  'page_speed',
  'meta_description',
  'alt_text'
  ...
  it('should return benchmark data for any valid industry/locale/size combination', async () => {
    await fc.assert(
      fc.asyncProperty(
        cohortBatchArb,
        requesterIdArb,
        async ({ metrics, industry, businessSize, locale }, requesterId) => {
          ...
          const res = await api.queryBenchmarks(
            { industry, locale, size: businessSize },
            requesterId
          );
  ...
  it('should include complete metadata in every benchmarks response', async () => {
    await fc.assert(
      fc.asyncProperty(
        cohortBatchArb,
        requesterIdArb,
        async ({ metrics, industry, locale }, requesterId) => {
          const benchmarkEngine = new BenchmarkEngine();
          await benchmarkEngine.addMetrics(metrics);
          const api = new IntelligenceAPI(
            benchmarkEngine,
  ...
  it('should return pattern data matching the platform filter', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        localeArb,
        requesterIdArb,
        async (platform, industry, locale, requesterId) => {
  ...
  it('should return effectiveness data for any valid recommendation_type/industry combination', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        localeArb,
        requesterIdArb,
        async (recType, industry, locale, requesterId) => {
          ...
          const res = await api.queryEffectiveness(
            { recommendation_type: recType, industry },
            requesterId
          );
  ...
  it('should log every benchmarks request with requester and filters', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        localeArb,
        requesterIdArb,
        async (industry, locale, requesterId) => {
          const api = new IntelligenceAPI(
            new BenchmarkEngine(),
            new PatternDiscoveryEngine(),
            new RecommendationEffectivenessTracker()
  ...
  it('should return empty array (not throw) for effectiveness when no data exists', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        industryArb,
        requesterIdArb,
        async (recType, industry, requesterId) => {
          const api = new IntelligenceAPI(
            new BenchmarkEngine(),
            new PatternDiscoveryEngine(),
            new RecommendationEffectivenessTracker()
