...
export async function monitorReputation() {
  ...
  for (const audit of audits) {
    try {
      logger.info(
        { auditId: audit.id, business: audit.businessName },
        '[ReputationMonitor] Checking audit'
      );
      ...
      const reputationFinding = result.findings.find(
        (f) => f.module === 'gbp' || f.module === 'reputation'
      );
