...
export function analyzeSchemaMarkup(html: string): SchemaAnalysis {
  ...
  const localBusinessItem = jsonLdItems.find((i) => {
    const t = i['@type'];
  ...
  const industryItem = jsonLdItems.find((i) => {
    ...
    return arr.some(
      (x) => typeof x === 'string' && INDUSTRY_SCHEMA_TYPES.some((ind) => x.includes(ind))
    );
  });
  ...
  const hasSitelinks = jsonLdItems.some(
    (i) => i['@type'] === 'WebSite' && (i as Record<string, unknown>).potentialAction
  );
  ...
  return {
    ...
    hasIndustrySpecific: {
      ...
      keyFields: industryItem
        ? { '@type': (industryItem as Record<string, unknown>)['@type'] }
        : undefined,
