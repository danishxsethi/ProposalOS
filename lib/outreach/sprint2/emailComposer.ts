...
function hasMetric(text: string): boolean {
  return /\b\d+(?:\.\d+)?(?:\s*(?:%|s|sec|seconds|ms|reviews?|rating|score|points?))?\b/i.test(
    text
  );
}
...
function pullFindings(input: ComposeSniperEmailInput): string[] {
  ...
  const breakdown = toObject<Record<string, unknown>>(
    input.painBreakdown,
    {}
  ) as PainBreakdownShape;
...
function pullCompetitor(qualificationEvidence: unknown): {
  name: string | null;
  reason: string | null;
} {
  const evidence = toObject<Record<string, unknown>>(
    qualificationEvidence,
    {}
  ) as CompetitorEvidence;
  const names = Array.isArray(evidence.competitorSignals?.competitorNames)
    ? evidence.competitorSignals?.competitorNames.filter(
        (item): item is string => typeof item === 'string' && item.trim().length > 0
      )
    : [];
  const reasons = Array.isArray(evidence.competitorSignals?.reasons)
    ? evidence.competitorSignals?.reasons.filter(
        (item): item is string => typeof item === 'string' && item.trim().length > 0
      )
    : [];
  return {
...
export function composeSniperEmail(input: ComposeSniperEmailInput): ComposedSniperEmail {
  ...
  const requiredFindingSnippets = [finding1, finding2]
