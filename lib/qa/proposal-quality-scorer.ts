...
function calculateClarity(proposal: ProposalResult): ClarityMetrics {
  ...
  const lengthScore = summary.length >= 50 && summary.length <= 500 ? 1 : Math.max(0, 1 - Math.abs(summary.length - 275) / 275);
  ...
  const avgWordsPerSentence = sentences.length > 0 
    ...
    : 0;
  const complexityScore = avgWordsPerSentence <= 25 ? 1 : Math.max(0, 1 - (avgWordsPerSentence - 25) / 25);
 ...
 */
function calculateSpecificity(proposal: ProposalResult, businessName?: string, industry?: string): SpecificityMetrics {
  ...
  const businessNameMentions = businessNameLower 
    ? (fullText.match(new RegExp(businessNameLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length 
    : 0;
  
  // Industry-specific terms
  const industryTerms = industry ? (INDUSTRY_TERMS[industry] || INDUSTRY_TERMS.general) : INDUSTRY_TERMS.general;
  const industryTermCount = (industryTerms || []).filter((term) => fullText.includes(term.toLowerCase())).length;
 ...
 */
function calculateActionability(proposal: ProposalResult, findings: FindingRuntime[]): ActionabilityMetrics {
  ...
  // Timeline clarity
  const hasTimeline = Object.values(proposal.tiers || {}).some((tier) => 
    tier.deliveryTime
  );
  
  // Ownership clarity (who does what)
  const fullText = JSON.stringify(proposal).toLowerCase();
  const hasOwnership = ['we will', 'our team', 'you will', 'your team', 'we\'ll', 'you\'ll'].some((phrase) => 
 ...
 */
function calculatePersuasiveness(proposal: ProposalResult, findings: FindingRuntime[]): PersuasivenessMetrics {
  const fullText = JSON.stringify(proposal).toLowerCase();
  ...
  const highImpactReferenced = Object.values(proposal.tiers || {}).some((tier) =>
    tier.findingIds?.some((id: string) => 
      findings.find((f) => f.id === id && f.impactScore >= 8)
    )
  );
  ...
  // Urgency language
  const urgencyWords = ['urgent', 'critical', 'immediately', 'asap', 'now', 'today', 'before', 'lose'];
  const urgencyCount = urgencyWords.filter((word) => fullText.includes(word)).length;
  
  // Social proof (reviews, ratings, competitors)
  const hasSocialProof = ['review', 'rating', 'competitor', 'ahead', 'behind', 'comparison'].some((word) =>
...
function calculatePersonalization(
  ...
  const businessNameInSummary = proposal.executiveSummary?.toLowerCase().includes(businessNameLower) || false;
