...
export default function TemplateEditorPage() {
  ...
  const [form, setForm] = useState({
    ...
    isDefault: false,
    introText:
      'We are excited to present our findings regarding your digital presence. This audit covers comprehensive performance, SEO, and user experience analysis.',
    outroText:
      'Ready to take the next step? We can help you implement these fixes and grow your business today.',
      ...
      <form id="template-form" onSubmit={handleSave} className="space-y-8">
        ...
        <section className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">
                Intro Text (Before Executive Summary)
              </label>
              <textarea
              ...
              <p className="text-xs text-slate-500 mt-1">
                Variables supported: {'{businessName}'}, {'{city}'}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">
                Outro Text (Footer/Closing)
              </label>
              <textarea
                className="w-full bg-slate-900 border border-slate-700 rounded p-3 text-white h-24 focus:outline-none focus:border-indigo-500"
