...
const industryArb = fc.constantFrom('dental', 'restaurant', 'ecommerce', 'legal', 'medical');
...
describe('Property 19: Competitor Metrics Localization', () => {
  ...
  it('should use Google for all standard locales', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.constantFrom(...googleLocales), industryArb, async (locale, industry) => {
        const localizer = new CompetitorAnalysisLocalizer();
        const competitor = {
          domain: 'example.com',
          name: 'Business',
