...
const demoData = {
    ...
    topFindings: [
        { severity: 'critical' as const, title: 'Google Business Profile missing 14 photos', details: 'Photos are a primary ranking factor for local search. Profiles with 100+ photos get 520% more directions requests.' },
        { severity: 'high' as const, title: 'Website loads in 4.8s on mobile — 67% slower than competitors', details: 'Every second of load time reduces conversion rate by 7%. Your mobile experience is currently a bottleneck for new patient acquisition.' },
        { severity: 'high' as const, title: 'Only 12 Google reviews vs competitor average of 47', details: 'Review volume and velocity signal trust to both Google and prospective patients. You are currently 4x behind the market leader.' },
        { severity: 'medium' as const, title: 'No schema markup detected on any page', details: 'Schema helps Google understand your services, location, and hours. Without it, you are missing out on rich snippets in search results.' },
        { severity: 'medium' as const, title: 'Social media profiles not linked from website', details: 'Cross-linking builds authority and helps customers verify your business across platforms.' },
    ],
    competitors: [
...
const testimonials = [
    {
        quote: "We had no idea our Google profile was only 62% complete. After fixing the issues Claraud found, our phone calls increased 40% in two months.",
        name: "Dr. Sarah Chen",
        business: "Riverside Dental, Saskatoon",
        stars: 5
    },
    {
        quote: "I was spending $2,000/month on ads with no idea my website was loading in 6 seconds. Claraud caught it in 30 seconds flat.",
        name: "Mike Torres",
        business: "Torres HVAC, Regina",
        stars: 5
    },
    {
        quote: "As an agency, we close 3x more deals now. We run a Claraud scan on every prospect before the first call.",
        name: "Priya Sharma",
    ...
    return (
        <div className="max-w-4xl mx-auto mb-16 px-4">
                ...
                <div className="flex border-b border-white/10 bg-white/5">
                    {(['overview', 'findings', 'competitors'] as const).map((tab) => (
                        <button
                            ...
                            className={`flex-1 py-4 text-sm font-semibold transition-all relative ${activeTab === tab ? 'text-white' : 'text-text-secondary hover:text-white hover:bg-white/5'
                                }`}
                        >
                            ...
                            >
                                <div className="relative aspect-square max-h-[300px] w-full mx-auto flex items-center justify-center bg-white/5 rounded-2xl border border-white/10">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                        <span className="text-4xl font-bold text-white leading-none">{demoData.overallScore}</span>
                                        <span className={`text-lg font-bold ${getScoreTextColor(demoData.overallScore)}`}>{demoData.letterGrade}</span>
                                    </div>
                                </div>
                                        ...
                                        >
                                            <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-white/5 text-left">
                                                <div className="flex items-center gap-3 w-full">
                                                    ...
                                                    <span className="text-sm font-medium text-white line-clamp-1">{finding.title}</span>
                                                </div>
                                            </AccordionTrigger>
                                        ...
                                        <thead>
                                            <tr className="border-b border-white/10">
                                                ...
                                                {demoData.competitors.map((comp) => (
                                                    <th key={comp.name} className={`py-3 px-2 font-bold ${comp.name === 'You' ? 'text-blue-400' : 'text-white'}`}>
                                                        {comp.name}
                                                    </th>
                                                ))}
                                            </tr>
                                        ...
                                        <tbody>
                                            {(['website', 'gbp', 'reviews'] as const).map((cat) => (
                                                <tr key={cat} className="border-b border-white/5">
                                                    <td className="py-3 px-2 text-text-secondary capitalize">{cat === 'gbp' ? 'Google Profile' : cat}</td>
                                                    {demoData.competitors.map((comp) => {
                                                        const score = comp.scores[cat];
                                                        return (
                                                            <td key={comp.name} className="py-3 px-2">
                                                                <div className={`inline-flex items-center justify-center w-10 h-10 rounded-lg font-bold ${getScoreBgColor(score)}/10 ${getScoreTextColor(score)}`}>
                                                                    {score}
                                                                </div>
                                                            </td>
                                                        );
            ...
            <div className="mt-8 text-center">
                <Button
                    onClick={() => document.getElementById('hero-scan')?.scrollIntoView({ behavior: 'smooth' })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto px-4 mt-16">
                {testimonials.map((t, idx) => (
                    ...
                    <div className="pt-6 border-t border-white/10 relative z-10">
                        <p className="text-sm text-text-secondary font-semibold">{t.name}, {t.business}</p>
