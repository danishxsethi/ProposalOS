...
export class PatternDiscoveryEngine {
  ...
  async trackPatternTrend(patternId: string): Promise<void> {
    ...
    const recentCount = stored.frequencyHistory.filter((h) => h.date >= thirtyDaysAgo).length;
  ...
  async getPatternTrendData(patternId: string, timeRange: TimeRange): Promise<TrendData> {
    ...
    const trend = this.computeTrend(values[values.length - 1] ?? 0, values[0] ?? 0);
  ...
  private matchesQuery(stored: StoredPattern, query: PatternQuery): boolean {
    if (
      ...
      !stored.affectedPlatforms.some((p) => p.toLowerCase() === query.platform!.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      !(stored.affectedPlugins ?? []).some((p) => p.toLowerCase() === query.plugin!.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      !stored.industries.some((i) => i.toLowerCase() === query.industry!.toLowerCase())
    ) {
      return false;
    }
   ...
   */
  private computeTrend(recent: number, previous: number): 'increasing' | 'stable' | 'decreasing' {
