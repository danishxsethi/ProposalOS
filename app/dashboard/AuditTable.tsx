...
export default function AuditTable() {
  ...
  const { data, error, isLoading } = useSWR(`/api/audits?${query.toString()}`, fetcher, {
            ...
            {isLoading ? (
              ...
              data?.audits?.map((audit: any) => (
                ...
                >
                  ...
                  <td className="py-4 text-sm">
                    {(audit.clientScore ?? audit.qaScore) !== undefined &&
                    (audit.clientScore ?? audit.qaScore) !== null ? (
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${
                          (audit.clientScore ?? audit.qaScore) >= 80
                            ? 'bg-green-500/20 text-green-400'
                            : (audit.clientScore ?? audit.qaScore) >= 60
                    ...
                    {audit.requiresHumanReview && (
                      <div className="text-[11px] text-amber-300 mt-1">
                        Needs human closeability review
                      </div>
                    )}
                  </td>
