...
async function fetchWebsiteSignals(website: string | null | undefined): Promise<WebsiteSignals> {
  ...
  const accessibilityScore =
    typeof accessibilityScoreRaw === 'number' ? Math.round(accessibilityScoreRaw * 100) : null;
...
async function fetchCompetitorSignals(
  ...
  const avgRating =
    competitorRatings.length > 0
      ? Number(
          (competitorRatings.reduce((sum, n) => sum + n, 0) / competitorRatings.length).toFixed(2)
        )
      : null;
  const avgReviewCount =
    competitorReviewCounts.length > 0
      ? Math.round(
          competitorReviewCounts.reduce((sum, n) => sum + n, 0) / competitorReviewCounts.length
        )
      : null;
...
function buildTopFindings(
  ...
  if (breakdown.websiteSpeed.score > 0) {
    findings.push(
      `Mobile performance is ${website.performanceScore ?? 'N/A'}/100 (LCP ${website.lcpSeconds ?? 'N/A'}s).`
    );
  }
...
export async function qualifyLead(
  ...
  const accessibilityScore =
    websiteSignals.accessibilityScore === null
      ? 5
