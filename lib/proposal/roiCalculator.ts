 ...
 */
export const INDUSTRY_ROI_BENCHMARKS: Record<
  string,
  {
...
> = {
  dentist: {
    avgOrderValue: 200,
    conversionRate: 0.03,
    targetBounceRate: 0.35,
    citation: 'Google/Deloitte 2023',
    contextHint: 'appointment bookings',
  },
  dental: {
    avgOrderValue: 200,
    conversionRate: 0.03,
  ...
  restaurant: {
    avgOrderValue: 35,
    conversionRate: 0.05,
    targetBounceRate: 0.4,
    citation: 'Google 2022',
    contextHint: 'online orders and reservations',
  },
  hvac: {
    avgOrderValue: 350,
    conversionRate: 0.025,
    targetBounceRate: 0.38,
    citation: 'Think With Google 2023',
    contextHint: 'service call requests',
  },
  plumber: {
    avgOrderValue: 350,
    conversionRate: 0.025,
    targetBounceRate: 0.38,
    citation: 'Think With Google 2023',
    contextHint: 'emergency service calls',
  },
  plumbing: {
    avgOrderValue: 350,
    conversionRate: 0.025,
  ...
  lawyer: {
    ...
    targetBounceRate: 0.45,
    citation: 'Clio 2023',
    contextHint: 'consultation requests',
  },
  legal: {
    avgOrderValue: 2000,
    conversionRate: 0.015,
  ...
  real_estate: {
    avgOrderValue: 8000,
    conversionRate: 0.01,
    targetBounceRate: 0.5,
    citation: 'NAR 2023',
    contextHint: 'listing inquiries and showings',
  },
  salon: {
    avgOrderValue: 85,
    conversionRate: 0.04,
    targetBounceRate: 0.38,
    citation: 'Square 2023',
    contextHint: 'appointment bookings',
  },
  spa: {
    avgOrderValue: 85,
  ...
  gym: {
    ...
    targetBounceRate: 0.4,
    citation: 'Google 2022',
    contextHint: 'membership signups',
  },
  fitness: {
    avgOrderValue: 50,
    conversionRate: 0.03,
  ...
  contractor: {
    ...
    conversionRate: 0.015,
    targetBounceRate: 0.45,
    citation: 'HomeAdvisor 2023',
    contextHint: 'project quote requests',
  },
  construction: {
    avgOrderValue: 5000,
    conversionRate: 0.015,
  ...
  default: {
    ...
    targetBounceRate: 0.4,
    citation: 'Google/Deloitte 2023',
    contextHint: 'online conversions',
  },
};
...
function getBounceRateFromLoadTime(loadTimeSeconds: number): number {
  if (loadTimeSeconds <= 1) return 0.2;
...
export function estimateMonthlyVisitors(
  ...
  if (reviewCount != null && reviewCount > 0) {
    ...
    return {
      visitors: Math.max(visitors, 100),
      source: `review_count * 150 (${reviewCount} reviews)`,
    };
  }
...
export function calculateBounceRateImpact(
  ...
  const benchmarks = (INDUSTRY_ROI_BENCHMARKS[industry?.toLowerCase()] ||
    INDUSTRY_ROI_BENCHMARKS.default)!;
...
export function calculateFindingROI(
  ...
  if (
    ...
    description.includes('load time') ||
    title.includes('lcp') ||
    title.includes('cls') ||
    title.includes('fcp') ||
    title.includes('tbt') ||
  ...
  // 2. GBP / Local SEO
    ...
    title.includes('local listing')
  ) {
    const { visitors } = estimateMonthlyVisitors(auditContext?.findings || [finding], industry);
    monthlyValue = Math.round(visitors * 0.1 * conversionRate * avgOrderValue);
    confidence = 'medium';
  ...
  // 3. Reviews / Reputation
    ...
    description.includes('reputation') ||
...
export function calculateTierROI(
  ...
  ratio: number;
  bounceImpact?: ReturnType<typeof calculateBounceRateImpact>;
} {
  ...
  const bounceImpact = hasSpeedFindings
    ? calculateBounceRateImpact(findingsForBounce, industry)
    : undefined;
