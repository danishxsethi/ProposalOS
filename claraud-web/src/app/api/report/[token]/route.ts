...
const CATEGORY_IDS = ['website', 'google', 'seo', 'reviews', 'social', 'competitors'];
    ...
    if (finding.metrics?.competitors) {
      for (const comp of finding.metrics.competitors) {
        competitors.push({
          ...
          url: comp.url || '',
          overallScore:
            typeof comp.overallScore === 'number'
...
function generateCategorySummary(findings: any[]): string {
  ...
  if (critical > 0)
    return `${critical} critical issue${critical > 1 ? 's' : ''} found — immediate attention needed.`;
...
function mapAuditToReportData(audit: any): ReportData {
  const findings = (audit.findings || []).flatMap((f: any) => {
    const categories = mapModuleToFrontendCategories(f.module);
  ...
  const categories = CATEGORY_IDS.map((catId) => {
    const catFindings = findings.filter((f: any) => f.category === catId);
    const avgImpact =
      catFindings.length > 0
        ? catFindings.reduce((sum: number, f: any) => sum + mapSeverityToNum(f.severity), 0) /
          catFindings.length
        : 5;
  ...
  const overallScore =
    typeof audit.overallScore === 'number'
      ? Math.round((audit.overallScore > 10 ? audit.overallScore / 10 : audit.overallScore) * 10) /
        10
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (audit) {
    ...
    const latestProposal =
      Array.isArray(audit.proposals) && audit.proposals.length > 0 ? audit.proposals[0] : null;
