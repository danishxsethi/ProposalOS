    ...
    for (const config of activeConfigs) {
      try {
        ...
        for (const prospect of prospects) {
          try {
            // Fetch the audit for this prospect by website URL
            const audit = prospect.website
              ? await prisma.audit.findFirst({
                where: { businessUrl: prospect.website, tenantId: config.tenantId },
                include: { findings: true },
              })
              : null;

            // Fetch most recent proposal for this audit
            const proposal = audit
              ? await prisma.proposal.findFirst({
                where: { auditId: audit.id, tenantId: config.tenantId },
            ...
            if (sendResult.status === 'sent') {
              ...
              await prisma.pipelineErrorLog.create({
                data: {
                  ...
                  metadata: { sendResult: sendResult as any },
                },
              });
            }
          } catch (err) {
