    ...
    <div className="w-full">
      {title && (
        <h2 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
          ...
          <Badge
            variant="outline"
            className="border-white/10 text-text-secondary bg-white/5 font-mono"
          >
            ...
            >
              ...
              >
                <div className="flex items-center justify-between w-full text-left gap-4 pr-4">
                  <div className="flex items-center gap-4 flex-1">
                    <Badge
                      variant="outline"
                      className={`capitalize px-3 py-0.5 font-bold ${getSeverityColor(finding.severity)}`}
                    >
                      {finding.severity}
                    </Badge>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="secondary"
                      className="bg-bg-input text-text-secondary border-none text-[10px] px-2 py-0.5 flex items-center"
                    >
                      {getComplexityIcon(finding.fixComplexity)}
                      <span className="uppercase tracking-wider">
                        {finding.fixComplexity.replace('-', ' ')}
                      </span>
                    </Badge>
                  </div>
                </div>
              ...
              <AccordionContent className="px-6 pb-6 pt-2 text-left">
                <div className="space-y-6">
                  <div>
                    <h4 className="text-[10px] uppercase tracking-widest text-text-secondary font-bold mb-2">
                      Impact
                    </h4>
      ...
      {findings.length > limit && (
        <div className="mt-8 text-center">
          <Button
            variant="outline"
            className="border-white/10 hover:bg-white/5 text-text-secondary px-8 rounded-full"
            asChild
          >
            <button
              onClick={() => {
