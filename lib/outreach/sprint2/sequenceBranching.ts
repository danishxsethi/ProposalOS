import { OutreachEmailStatus, OutreachEmailType, OutreachLeadStage } from '@prisma/client';
...
export function evaluateSequenceBranching(
  ...
  if (history.totalClicks > 0) {
    return {
      kind: 'trigger_closing_agent',
      reason: 'Prospect clicked a link (Scorecard/Proposal)',
    };
  }
  ...
  if (history.totalOpens > 0 && history.lastOpenedAt) {
    ...
    if (hoursSinceLastOpen < 48) {
      ...
      return {
        kind: 'continue',
        reason: `Opened ${Math.round(hoursSinceLastOpen)}h ago, waiting for click`,
      };
    }
  }
