...
async function runGoldenTests() {
  ...
  for (let i = 0; i < GOLDEN_BUSINESSES.length; i++) {
    ...
    try {
      ...
      console.log(
        `   • Executive summary: ${proposal.proposal.executiveSummary.substring(0, 80)}...`
      );
      console.log(
        `   • Pricing: $${proposal.proposal.pricing.essentials}/$${proposal.proposal.pricing.growth}/$${proposal.proposal.pricing.premium}`
      );
  ...
  results.forEach((result, i) => {
    ...
    if (result.status === 'pass') {
      console.log(
        `   Findings: ${result.findings}, Cost: $${result.cost}, Duration: ${result.duration}ms`
      );
    } else {
