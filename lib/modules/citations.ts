...
export async function runCitationsModule(
  ...
  logger.info(
    ...
    '[Citations] Starting citation analysis'
  );
...
async function checkFacebook(
  ...
  try {
    ...
    const facebookPage = data.organic_results.find(
      (r: any) => r.link?.includes('facebook.com') && !r.link?.includes('/posts/')
    );
...
function calculateNAPConsistency(
  ...
  listings.forEach((listing) => {
    ...
    if (listedAddress && expectedAddress && listedAddress.includes(expectedAddress.slice(0, 15))) {
      ...
      listedAddress &&
      expectedAddress &&
      !listedAddress.includes(expectedAddress.slice(0, 15))
    ) {
...
function generateCitationFindings(
  ...
  if (yelpListing && !yelpListing.found) {
    findings.push({
      ...
      description:
        'Business is not found on Yelp. Over 50% of consumers check Yelp before choosing a local business. Missing this listing costs you customers.',
      impactScore: 7,
  ...
  if (bbbListing && !bbbListing.found) {
    findings.push({
      ...
      description:
        'Business is not found on Better Business Bureau. BBB accreditation builds trust and improves local search visibility.',
      impactScore: 4,
  ...
  if (yellowPagesListing && !yellowPagesListing.found) {
    findings.push({
      ...
      description:
        'Business is not found on Yellow Pages. While less critical than Yelp, it is still a trusted directory that drives local traffic.',
      impactScore: 3,
      confidenceScore: normalizeConfidence(85, '0-100'),
      evidence: [
        {
  ...
  if (facebookListing && !facebookListing.found) {
    findings.push({
      ...
      description:
        'Business does not have a Facebook page. With 2.9 billion users, Facebook is critical for local visibility and customer engagement.',
      impactScore: 5,
      confidenceScore: normalizeConfidence(90, '0-100'),
      evidence: [
        {
