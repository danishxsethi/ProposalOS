...
async function createTestTenant(): Promise<string> {
  return runWithTenantBypass('test-setup:create-test-tenant-prop', async () => {
...
async function cleanupTestData() {
  await runWithTenantBypass('test-cleanup:db-cleanup-orchestrator', async () => {
...
describe('Pipeline Orchestrator Property Tests', () => {
  ...
  describe('Property 32: FIFO queue ordering', () => {
    ...
    it('FIFO order is maintained per tenant', async () => {
      ...
      const tenant1Results = await runWithTenantAsync(tenant1, async () => {
      ...
      const tenant2Results = await runWithTenantAsync(tenant2, async () => {
        return prisma.prospectLead.findMany({
          where: {
            tenantId: tenant2,
            pipelineStatus: 'discovered',
