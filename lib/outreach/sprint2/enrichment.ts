...
// FIX-21: Generic email address prefixes to avoid
const GENERIC_EMAIL_PREFIXES = [
    'info@', 'contact@', 'hello@', 'admin@', 'support@',
    'sales@', 'office@', 'enquiries@', 'enquiry@', 'team@',
    'mail@', 'noreply@', 'no-reply@', 'webmaster@', 'help@',
];
...
export async function runEnrichmentWaterfall(lead: EnrichmentLeadInput): Promise<EnrichmentResult> {
    ...
    let genericFallback: EnrichedContact | null = null;

    for (const runner of runners) {
        const result = await runner(lead);
        runs.push(result);
        const candidate = mergeContact(mergedContact, result.contact);

        if (result.status === 'SUCCESS' && candidate?.email) {
            if (isGenericEmail(candidate.email)) {
                // FIX-21: Keep as fallback, but continue looking for a decision-maker email
                genericFallback = candidate;
                logger.info({ email: candidate.email }, 'Enrichment: generic email found, continuing waterfall');
            } else {
                // We have a real decision-maker email — stop here
                mergedContact = candidate;
                break;
            }
        } else {
            mergedContact = candidate;
    ...
    // FIX-21: Use generic email only as last resort, flagged appropriately
    if (!mergedContact?.email && genericFallback?.email) {
        mergedContact = genericFallback;
        logger.info({ email: mergedContact?.email }, 'Enrichment: using generic email as last resort fallback');
    }
    ...
    const isGenericResult = isGenericEmail(mergedContact?.email);
    ...
    logger.info({
        ...
        isGenericEmail: isGenericResult,
        totalCostCents,
