...
import { enrichProspect } from '@/lib/pipeline/waterfallEnrichment';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    try {
      const pendingLeads = await prisma.prospectLead.findMany({
        where: { status: 'ENRICH_PENDING' },
        take: 20,
      });

      logger.info({ count: pendingLeads.length }, 'Processing pending enrichments');

      for (const lead of pendingLeads) {
        enrichProspect(lead.id).catch(err => {
          logger.error({ leadId: lead.id, error: err }, 'Enrichment failed');
        });
      }
    } catch (enrichErr) {
      logger.error({ error: enrichErr }, 'Failed to query or process pending enrichments');
    }
