...
export function generateComparison(
  ...
  const prospectRank = competitors.length > 0 ? rankProspect(prospect, competitors, metricKeys) : 1;
  ...
  if (losingCategories.length > 0) {
    ...
    if (worst === 'Load Time' || worst === 'PageSpeed' || worst === 'Mobile Score') {
      summaryParts.push(
        `Your website is slower than ${competitors.length > 1 ? 'all competitors' : 'your competitor'}.`
      );
    } else {
  ...
  for (const { key, label, format, lowerBetter } of metricConfigs) {
    ...
    const compVals = competitors
      .map((c) => ({ name: c.name, val: getMetricValue(c, key) }))
      .filter((x) => x.val > 0 || key === 'loadTimeSeconds');
    if (compVals.length === 0) continue;

    const bestCompVal = lowerBetter
      ? Math.min(...compVals.map((x) => x.val))
      : Math.max(...compVals.map((x) => x.val));
    ...
    const prospectStatus: 'win' | 'lose' | 'tie' = prospectWinsMetric
      ? 'win'
      : prospectTie
        ? 'tie'
        : 'lose';
  ...
  for (const row of comparisonTableRows) {
    if (row.prospectStatus === 'win' && whereAhead.length < 3) {
      ...
      whereAhead.push(
        `Your ${row.metric.toLowerCase()} (${row.prospectValue}) beats ${comp?.name || 'competitors'} (${comp?.value ?? '—'})`
      );
    }
    if (row.prospectStatus === 'lose' && whereBehind.length < 3) {
      const bestComp =
        row.competitorValues.find((c) => c.status === 'win') || row.competitorValues[0];
      if (bestComp) {
        whereBehind.push(
          `${bestComp.name}'s ${row.metric.toLowerCase()} (${bestComp.value}) beats yours (${row.prospectValue})`
        );
      }
    }
  }
  ...
  if (whereBehind.length === 0 && biggestGap) {
    ...
    if (LOWER_IS_BETTER.has(metricKey)) {
      whereBehind.push(
        `Your site loads in ${prospectScore}s vs ${competitorName}'s ${bestCompetitorScore}s — they're capturing impatient visitors`
      );
    } else {
      whereBehind.push(
        `${competitorName} scores ${bestCompetitorScore} on ${category} vs your ${prospectScore}`
      );
    }
  }
  ...
  if (prospect.loadTimeSeconds != null && prospect.loadTimeSeconds > 3) {
    const fastest = Math.min(
      ...competitors.map((c) => c.loadTimeSeconds ?? 999).filter((v) => v > 0),
      999
    );
