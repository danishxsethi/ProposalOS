...
export async function runGBPModule(input: GBPModuleInput, tracker?: CostTracker): Promise<LegacyAuditModuleResult> {
    ...
    try {
        ...
        logger.error({ error }, '[GBPModule] Analysis failed');
        // Return structured DATA_UNAVAILABLE fallback
        return {
            moduleId: 'gbp',
            status: 'failed',
            ...
            data: {
                findings: [{
                    ...
                    category: 'System',
                    title: 'GBP Data Unavailable',
                    description: `API Timeout/Failure: Unable to fetch Google Places profile. Assume fallback required.`,
                    impactScore: 0,
                    confidenceScore: 0,
                    evidence: [{ type: 'text', value: 'Google Places search failed', label: 'Error' }],
                    metrics: { available: false },
                    effortEstimate: 'LOW',
                    recommendedFix: ['Verify business name and location formatting']
                }]
            }
        };
    }
