...
function recommendTier(findings: Finding[]): 'starter' | 'growth' | 'premium' {
...
export async function runProposalPipeline(
  ...
  console.log(
    `[ProposalPipeline] Generating proposal for ${businessName}...${playbook ? ` (vertical: ${playbook.id})` : ''}`
  );
  ...
  console.log(
    `[ProposalPipeline] Detected business size: ${detectedBusinessSize} (Max reviews: ${maxReviewCount})`
  );
  ...
  console.log(
    `[ProposalPipeline] Pricing: $${pricing.essentials}/$${pricing.growth}/$${pricing.premium}${multiplier !== 1 ? ` (${multiplier}x)` : ''}`
  );
