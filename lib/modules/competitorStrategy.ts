...
export async function runCompetitorStrategyModule(
  ...
  logger.info(
    { businessName: input.businessName, competitor: input.competitorName },
  ...
  try {
    ...
    const ourData = {
      // We assume the main audit runs elsewhere and we might need to pass this or just focus on "What they do"
    ...
    logger.error(
      { error, businessName: input.businessName },
      '[CompetitorStrategy] Analysis failed'
    );
 ...
 * Generate Findings
 */
function generateStrategyFindings(
  ...
  analysis.insights
    .filter((i) => i.advantage === 'Competitor')
    .slice(0, 3)
    .forEach((insight) => {
      findings.push({
        type: 'VITAMIN',
