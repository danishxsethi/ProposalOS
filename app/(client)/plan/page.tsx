    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="relative border-l-2 border-slate-200 ml-4 space-y-12">
        ...
        <TimelineItem
          ...
          date="Week 7+"
          status="future"
          items={['Paid Ads Setup', 'Advanced Analytics', 'Quarterly Review']}
        />
      </div>
    </div>
  );
...
function TimelineItem({ phase, date, status, items }: any) {
  ...
  return (
    <div className="relative pl-8">
      <div
        className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white ${color}`}
      />
      <div className="font-bold text-lg text-slate-800 mb-1">
        {phase} <span className="text-sm font-normal text-slate-500 ml-2">({date})</span>
      </div>
        ...
        <ul className="space-y-3">
          {items.map((item: string, i: number) => (
            <li key={i} className="flex items-center gap-3 text-slate-600">
              ...
              <span className={status === 'complete' ? 'line-through text-slate-400' : ''}>
                {item}
              </span>
            </li>
          ))}
