...
describe('queryBenchmarks', () => {
  it('returns a response with correct structure', async () => {
    ...
    const res = await api.queryBenchmarks(
      { industry: 'dental', locale: 'en-US', size: 'small' },
      'svc-1'
    );
  ...
  it('throws when industry is missing', async () => {
    const api = makeAPI();
    await expect(api.queryBenchmarks({ industry: '', locale: 'en-US' }, 'svc-1')).rejects.toThrow(
      'industry'
    );
  });
  ...
  it('throws when locale is missing', async () => {
    const api = makeAPI();
    await expect(api.queryBenchmarks({ industry: 'dental', locale: '' }, 'svc-1')).rejects.toThrow(
      'locale'
    );
  });
...
describe('queryEffectiveness', () => {
  ...
  it('returns empty data array when no effectiveness data exists', async () => {
    ...
    const res = await api.queryEffectiveness(
      { recommendation_type: 'schema_markup', industry: 'dental' },
      'svc-1'
    );
...
describe('rate limiting', () => {
  ...
  it('throws when rate limit is exceeded on queryBenchmarks', async () => {
    ...
    await expect(
      api.queryBenchmarks({ industry: 'dental', locale: 'en-US' }, 'svc-x')
  ...
  it('rate limits are per-requester (different requesters are independent)', async () => {
    ...
    await expect(
      api.queryBenchmarks({ industry: 'dental', locale: 'en-US' }, 'svc-other')
