...
const REVIEW_WIDGET_SIGNATURES = [
  'google reviews',
  'yelp',
  'trustpilot',
  'reviews',
...
function getElementApproxHeight(
  $: cheerio.CheerioAPI,
  el: { tagName?: string } & Record<string, unknown>
): number {
...
function estimateCumulativeHeight(
  $: cheerio.CheerioAPI,
  elements: Array<{ tagName?: string } & Record<string, unknown>>
): number {
  let total = 0;
...
function isAboveFold(
  $: cheerio.CheerioAPI,
  el: { tagName?: string },
  precedingElements: Array<{ tagName?: string }>
): boolean {
 ...
 */
export function detectConversionElements(
  html: string,
  baseUrl?: string
): ConversionDetectionResult {
  ...
  if (!hasTel) {
    ...
    recommendations.push(
      "Add a click-to-call phone link in the header/hero — mobile visitors can't contact you with one tap (your highest-intent traffic)"
    );
  } else if (phoneLocation === 'below-fold') {
    recommendations.push(
      'Move phone number above the fold — most mobile users decide in the first 3 seconds'
    );
  }
  ...
  const formLibs = EMAIL_SIGNATURES.concat([
    'gravity',
    'typeform',
    'hubspot',
    'calendly',
    ...
    'ninja-forms',
    'contact-form',
  ]).some((sig) => htmlLower.includes(sig));
  ...
  if (!hasForm && !formLibs) {
    ...
    recommendations.push(
      'Add a contact form — visitors ready to inquire need an easy way to reach you'
    );
  }
  ...
  if (!hasCta) {
    ...
    recommendations.push(
      'Add prominent CTAs (Book, Schedule, Call, Get Quote) — visitors need a clear next step'
    );
  } else if (ctaLocation === 'below-fold') {
    recommendations.push(
      'Add at least one CTA above the fold — capture intent before users scroll away'
    );
  }
  ...
  if (!hasChat) {
    recommendations.push(
      'Consider a chat widget (Intercom, Tidio, etc.) — capture leads who prefer instant messaging'
    );
  }
  ...
  if (!hasBooking) {
    recommendations.push(
      'Add online booking (Calendly, Acuity, etc.) — reduce friction for appointment-based businesses'
    );
  }

  // 6. Social proof (reviews, testimonials)
  const hasReviewWidget = REVIEW_WIDGET_SIGNATURES.some(
    (sig) => htmlLower.includes(sig) || bodyText.includes(sig)
  );
  const hasTestimonialClass =
    $('[class*="testimonial"], [class*="review"], [class*="rating"]').length > 0;
  ...
  const hasEmailForm =
    EMAIL_SIGNATURES.some((sig) => htmlLower.includes(sig)) ||
  ...
  const hasMapEmbed =
    ...
    htmlLower.includes('embed');
  const hasDirectionsLink =
    $('a[href*="maps.google"], a[href*="google.com/maps"], a[href*="directions"]').length > 0;
  ...
  const criticalDetected = elements.filter(
    (e) =>
      [
        'Phone number (click-to-call)',
        'Contact form',
        'CTA buttons (Book, Schedule, Call, etc.)',
      ].includes(e.name) && e.detected
  ).length;
  const score = Math.min(
    100,
    Math.round((detectedCount / elements.length) * 70 + (criticalDetected / 3) * 30)
  );
