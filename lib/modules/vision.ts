...
export async function runVisionModule(
  ...
  logger.info(
    { auditId: input.auditId, numScreenshots: input.screenshots.length },
    '[VisionModule] Executing Prompt 20 Vision Analysis'
  );
  ...
  if (imagesToAnalyze.length === 0) {
    logger.warn(
      ...
      '[VisionModule] No valid base64 screenshots to analyze'
    );
...
DO NOT wrap the response in markdown codeblocks. Just raw JSON. Keep it professional and persuasive.`;
  ...
  try {
    ...
    logger.info(
      { auditId: input.auditId, findingsGenerated: findings.length },
      '[VisionModule] Complete'
    );
