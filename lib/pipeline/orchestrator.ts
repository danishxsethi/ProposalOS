...
import { scoreProspect } from '@/lib/intelligence/predictiveScoring';
import type { ProspectFeatures } from '@/lib/intelligence/types';
...
/**
 * Get prospects for a given stage, sorted by predictive close probability (descending).
 *
 * For the OUTREACH and CLOSING stages, prospects are scored by the predictive
 * lead scoring model and sorted by `closeProbability` descending so the
 * highest-value prospects are processed first. Each prospect is annotated with
 * `closeProbability` and `modelVersion` from the scoring result.
 *
 * For all other stages, FIFO ordering (createdAt asc) is preserved.
 *
 * Requirements: 13.6
 */
async function getProspectsForStage(
  ...
  // Fetch a larger candidate pool for stages that use predictive prioritization
  const usesPredictiveScoring =
    stage === PipelineStage.OUTREACH || stage === PipelineStage.CLOSING;

  const candidateLimit = usesPredictiveScoring ? batchSize * 3 : batchSize;
  ...
  const prospects = await prisma.prospectLead.findMany({
    ...
    orderBy: { createdAt: 'asc' }, // FIFO as base ordering
    take: candidateLimit,
  });

  if (!usesPredictiveScoring || prospects.length === 0) {
    return prospects;
  }

  // Score each prospect and annotate with closeProbability + modelVersion
    ...
    prospects.map(async (prospect) => {
      try {
        const features: ProspectFeatures = {
          ...
          geoRegion: prospect.city ?? 'unknown',
          reviewCount: prospect.reviewCount ?? undefined,
        };

        const leadScore = await scoreProspect(features);

        return {
          ...prospect,
          closeProbability: leadScore.closeProbability,
          modelVersion: leadScore.modelVersion,
        };
      } catch {
        // If scoring fails, assign a neutral score so the prospect is still processed
        return {
          ...prospect,
          closeProbability: 50,
          modelVersion: 'scoring-unavailable',
        };
      }
    })
  );

  // Sort by closeProbability descending, then take the requested batch size
  scored.sort((a, b) => (b.closeProbability ?? 0) - (a.closeProbability ?? 0));
