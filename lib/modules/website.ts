...
export async function runWebsiteModule(
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        { error: schemaErr },
        '[WebsiteModule] Schema/conversion analysis failed, skipping'
      );
    }

    const allFindings = [...crawlerResult.findings, ...psiResult.findings];
...
function generateSchemaFindings(
  schema: ReturnType<typeof analyzeSchemaMarkup>,
  url: string
): Finding[] {
...
async function getPageSpeedFindings(url: string): Promise<PageSpeedResult> {
  ...
  try {
    ...
    const cwvFull = extractCoreWebVitalsFromAudits(
      audits as Parameters<typeof extractCoreWebVitalsFromAudits>[0]
    );
    ...
    if (cwvFull.inp && cwvFull.inp.rating !== 'good') {
      ...
      if (cwvFull.inp.rating === 'poor') {
        ...
        findings.push({
          ...
          impactScore: 4,
          confidenceScore: normalizeConfidence(8.5, '1-10'),
          evidence: [
            createEvidence({
              pointer,
              source: 'pagespeed_v5',
