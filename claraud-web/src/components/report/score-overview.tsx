...
import { Award, Globe, Search, Share2, Shield, Smartphone, Zap } from 'lucide-react';
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-3">
                <div
                  className={`p-2 rounded-lg bg-white/5 border border-white/10 group-hover:border-blue-500/30 transition-colors ${getScoreTextColor(cat.score)}`}
                >
                  {CATEGORY_ICONS[cat.id] || <Globe className="w-4 h-4" />}
                </div>
                <span className="font-semibold text-white tracking-tight">{cat.name}</span>
              </div>
              <span className={`font-bold ${getScoreTextColor(cat.score)}`}>
