...
import { Activity, DollarSign, Download, Percent, TrendingUp, Users } from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  const statCards = [
    ...
    {
      title: 'Est. MRR',
      value: `$${(analytics?.overview?.mrr || 0).toLocaleString()}`,
      icon: TrendingUp,
      color: 'text-emerald-500',
    },
    {
      title: 'Audits (30d)',
      value: analytics?.overview?.auditsThisMonth || 0,
      icon: Activity,
      color: 'text-blue-500',
    },
    {
      title: 'Proposals Sent',
      value: analytics?.overview?.proposalsSent || 0,
      icon: Users,
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1">
            Deep dive into your sales and auditing performance.
          </p>
        </div>
          ...
          <CardContent className="h-[250px] flex items-end gap-1 px-4 pb-4">
            {isLoading ? (
              ...
              analytics?.charts?.audits.map((a: any, i: number) => (
                <div
                  key={i}
                  className="flex-1 bg-blue-500/50 hover:bg-blue-400 rounded-t-sm transition-all"
                  style={{ height: `\${Math.max(2, (a.count / 5) * 100)}%` }}
            ...
            {isLoading ? (
              ...
              <div className="w-full flex items-end gap-1 h-full">
                {analytics?.charts?.revenue.map((r: any, i: number) => (
                  <div
                    key={i}
                    className="flex-1 bg-green-500/40 hover:bg-green-400 rounded-t-sm transition-all"
                    style={{ height: `\${Math.max(2, (r.amount / 5000) * 100)}%` }}
                    title={`\${r.date}: $\${r.amount}`}
                  />
                ))}
              </div>
            )}
        ...
        <Card className="bg-bg-secondary border-white/10 h-64">
          ...
          <CardContent className="space-y-4 pt-4">
            {isLoading
              ? Array(4)
                  .fill(0)
                  .map((_, i) => <Skeleton key={i} className="h-6 w-full bg-white/10" />)
              ...
              <div className="space-y-2">
                <div className="relative h-10 w-full bg-blue-500/20 rounded-md flex items-center justify-between px-4 mt-2">
                  ...
                  <span className="text-sm font-bold text-white">
                    {analytics?.overview?.totalProposals || 0}
                  </span>
                </div>
                <div className="relative h-10 w-[80%] mx-auto bg-purple-500/20 rounded-md flex items-center justify-between px-4">
                  ...
                  <span className="text-sm font-bold text-white">
                    {Math.round((analytics?.overview?.totalProposals || 0) * 0.8)}
                  </span>
                </div>
                <div className="relative h-10 w-[60%] mx-auto bg-green-500/20 rounded-md flex items-center justify-between px-4">
                  ...
                  <span className="text-sm font-bold text-white">
                    {analytics?.overview?.acceptedProposals || 0}
                  </span>
                </div>
              </div>
            )}
