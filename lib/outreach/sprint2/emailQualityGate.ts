...
function countMetricMentions(text: string): number {
  const matches = text.match(
    /\b\d+(?:\.\d+)?(?:\s*(?:%|s|sec|seconds|ms|reviews?|rating|score|points?))?\b/gi
  );
...
function countFindingsReferenced(body: string, requiredFindingSnippets: string[]): number {
  ...
  const snippets = requiredFindingSnippets.map((snippet) => normalize(snippet)).filter(Boolean);
...
export function checkSniperEmailQuality(input: SniperEmailQualityInput): SniperEmailQualityResult {
  ...
  if (wordCount > TARGET_MAX_WORDS)
    hardFails.push(`Word count ${wordCount} exceeds ${TARGET_MAX_WORDS}`);
  if (readabilityGrade > TARGET_MAX_GRADE)
    hardFails.push(`Reading grade ${readabilityGrade} is above ${TARGET_MAX_GRADE}`);
  ...
  const issues: string[] = [];
  if (wordCount > 65 && wordCount <= TARGET_MAX_WORDS)
    issues.push('Copy is near the 80-word limit');
  if (readabilityGrade > 4.5 && readabilityGrade <= TARGET_MAX_GRADE)
    issues.push('Readability is close to hard threshold');
