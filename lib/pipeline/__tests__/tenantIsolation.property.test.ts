...
import {
  ...
  withTestSystemSetup,
  withTestTenant,
} from '@/lib/__tests__/utils/testPrincipal';
/**
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (tenantIds) => {
          ...
          // Tenant IDs generated by fast-check do not imply database fixtures.
          // Create explicit tenants, then verify data only through scoped APIs.
          const createdTenantIds: string[] = [];
          for (const requestedId of tenantIds) {
            const tenant = await createTestTenant(`Isolation ${requestedId}`);
            createdTenantIds.push(tenant.id);
            await withTestTenant(tenant.id, () => prisma.pipelineConfig.upsert({
          ...
          for (const tenantId of createdTenantIds) {
            ...
            if (config) {
              ...
              const otherTenantIds = createdTenantIds.filter((id) => id !== tenantId);
          ...
          await withTestSystemSetup(async () => {
            await prisma.pipelineConfig.deleteMany({ where: { tenantId: { in: createdTenantIds } } });
