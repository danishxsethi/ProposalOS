...
export class LocaleConfigManager {
  ...
  async getLocaleConfig(locale: string): Promise<LocaleConfig | null> {
    ...
    try {
      const result = await query('SELECT * FROM locale_configs WHERE locale = $1', [locale]);
  ...
  async getSupportedLocales(): Promise<string[]> {
    ...
    // Load from database
    try {
      const result = await query('SELECT locale FROM locale_configs ORDER BY locale');
  ...
  async validateConfigCompleteness(locale: string): Promise<ValidationResult> {
    ...
    if (config.currency && !/^[A-Z]{3}$/.test(config.currency)) {
      errors.push(
        `Invalid currency format: ${config.currency}. Must be 3-letter code (e.g., USD, EUR)`
      );
    }
