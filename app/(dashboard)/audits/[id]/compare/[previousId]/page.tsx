      ...
      <header className="mb-10">
        ...
        <div className="flex items-center gap-4 mt-6">
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            <div className="text-slate-400 text-xs uppercase font-bold text-center">
              Score Change
            </div>
            <div
              className={`text-3xl font-bold text-center ${scoreDiff > 0 ? 'text-green-400' : scoreDiff < 0 ? 'text-red-400' : 'text-slate-400'}`}
            >
              {scoreDiff > 0 ? '+' : ''}
              {scoreDiff}
            </div>
          ...
          <div className="bg-green-500/10 p-4 rounded-xl border border-green-500/20">
            <div className="text-green-400 text-xs uppercase font-bold text-center">
              Issues Fixed
            </div>
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            ...
            <div className="text-3xl font-bold text-slate-100 text-center">
              {newFindings.length}
            </div>
          </div>
        </div>
      ...
      <div className="space-y-8">
          ...
          <section>
            ...
            <div className="grid gap-4">
              {improved.map((f: any) => (
                <div
                  key={f.id}
                  className="bg-green-900/10 border border-green-500/20 p-4 rounded-lg flex justify-between items-center opacity-75"
                >
                  <div>
                    <div className="font-semibold text-green-200 line-through decoration-green-500/50">
                      {f.title}
                    </div>
              ...
              {newFindings.map((f: any) => (
                <div key={f.id} className="bg-slate-800 border border-slate-700 p-4 rounded-lg">
                  <div className="flex justify-between">
                    ...
                    <div className="text-red-400 font-bold bg-red-400/10 px-2 rounded text-xs h-fit">
                      {f.impactScore}/10
                    </div>
                  </div>
        ...
        <section>
          ...
          <div className="grid gap-4 opacity-75">
            {unchanged.slice(0, 5).map(
              (
                f: any // Show only top 5 unchanged to avoid clutter
              ) => (
                <div
                  key={f.id}
            ...
            {unchanged.length > 5 && (
              <p className="text-center text-slate-500 text-sm">
                ...and {unchanged.length - 5} more unchanged items
              </p>
            )}
          </div>
        </section>
