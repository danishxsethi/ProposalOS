...
import { generateGBPFindings, generateWebsiteFindings } from '../findingGenerator';
...
describe('Finding Generator', () => {
  ...
  describe('generateGBPFindings', () => {
    it('should report missing website as PAINKILLER', () => {
      ...
      const websiteFinding = findings.find(
        (f) => f.category === 'conversion' && f.title.includes('No website')
      );
