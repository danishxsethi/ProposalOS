...
import { Archive, ExternalLink, RefreshCw, Search } from 'lucide-react';
...
export default function ProposalsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/proposals?search=${encodeURIComponent(search)}&status=${statusFilter}`
      );
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 rounded-full text-xs font-medium border ${styles[status] || styles.DRAFT}`}
      >
        {status}
      </span>
    );
                    ...
                    .map((_, i) => (
                      <tr key={i} className="bg-bg-secondary">
                        ...
                        <td className="px-6 py-4 text-right">
                          <Skeleton className="h-8 w-8 ml-auto bg-white/10" />
                        </td>
                      </tr>
                    ))
                  ...
                  data.proposals.map((proposal: any, i: number) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-white">
                        {proposal.tierChosen ? (
                          <span className="capitalize">{proposal.tierChosen}</span>
                        ) : (
                          '-'
                        )}
                      ...
                      <td className="px-6 py-4 text-text-secondary">
                        {proposal.lastViewedAt
                          ? format(new Date(proposal.lastViewedAt), 'MMM d, h:mm a')
                          : 'Never'}
                      </td>
                      <td className="px-6 py-4 text-text-secondary">
