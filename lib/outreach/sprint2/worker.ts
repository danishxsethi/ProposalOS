...
async function upsertDiscoveredLead(
  job: ProspectDiscoveryJob,
  business: Awaited<ReturnType<typeof discoverBusinesses>>['businesses'][number]
  ...
  for (const lead of leads) {
    ...
    await prisma.prospectLead.update({
      ...
      data: {
        ...
        disqualifiedReason: qualification.qualified
          ? null
          : `Pain score ${qualification.painScore} below threshold ${job.painThreshold}`,
      },
    });
...
async function enrichLeads(job: ProspectDiscoveryJob, limit: number): Promise<number> {
  ...
  for (const lead of leads) {
    ...
    const hasContact = Boolean(
      enrichment.contact?.email || enrichment.contact?.name || enrichment.contact?.linkedin
    );
...
async function processOneJob(
  job: ProspectDiscoveryJob,
  options: Required<ProcessJobsOptions>
): Promise<JobProcessSummary> {
