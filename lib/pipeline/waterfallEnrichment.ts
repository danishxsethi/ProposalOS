...
import { verifyEmailWithZeroBounce } from '@/lib/discovery/zeroBounce';
...
/**
 * Returns true if the provider result contains a verified email via ZeroBounce.
 */
export async function hasVerifiedEmail(result: ProviderResult): Promise<boolean> {
  if (typeof result.email !== 'string' || result.email.length === 0) {
    return false;
  }
  const zbResult = await verifyEmailWithZeroBounce(result.email);
  return zbResult.isValid;
}
...
export async function enrichProspect(
  ...
  for (const providerName of PROVIDER_ORDER) {
    ...
    try {
      ...
      if (await hasVerifiedEmail(result)) {
