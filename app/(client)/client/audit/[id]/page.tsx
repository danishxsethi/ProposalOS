      ...
      <div className="space-y-4">
        {audit.findings.map((finding) => {
          ...
          return (
            <div
              key={finding.id}
              className={`bg-white border p-6 rounded-xl flex justify-between items-start ${
                status === 'fixed' ? 'border-green-200 bg-green-50/30' : 'border-slate-200'
              }`}
            >
              <div className="max-w-xl">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3
                    className={`font-bold text-lg ${status === 'fixed' ? 'text-green-800 line-through decoration-green-800/30' : 'text-slate-800'}`}
                  >
                    {finding.title}
                  </h3>
                </div>
                ...
                {finding.recommendedFix && (
                  <div className="text-xs text-slate-500 bg-slate-50 p-3 rounded">
                    ...
                    {JSON.parse(JSON.stringify(finding.recommendedFix))[0]}
                  </div>
                )}
              </div>
