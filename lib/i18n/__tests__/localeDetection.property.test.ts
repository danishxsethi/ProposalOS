...
describe('Locale Detection Properties', () => {
  ...
  describe('Property 3: Locale Storage Completeness', () => {
    ...
    it('should store detected locale in audit context for downstream use', () => {
      fc.assert(
        fc.property(fc.oneof(...SUPPORTED_LOCALES.map((l) => fc.constant(l))), (locale: string) => {
      ...
      fc.assert(
        fc.property(fc.oneof(...SUPPORTED_LOCALES.map((l) => fc.constant(l))), (locale: string) => {
          const config: LocaleConfig = {
            ...
            tone: fc.sample(
              fc.oneof(fc.constant('formal'), fc.constant('casual'), fc.constant('professional')),
              1
            )[0] as any,
    ...
    it('should have valid confidence score between 0 and 1', () => {
      fc.assert(
        fc.property(
          ...
          fc.oneof(
            fc.constant('tld'),
            fc.constant('hreflang'),
            fc.constant('gbp'),
            fc.constant('ip_geolocation'),
  ...
  describe('Property 7: Fallback Chain Validity', () => {
    ...
    it('should have valid fallback chain with detected locale included', () => {
      fc.assert(
        fc.property(
          fc.oneof(...SUPPORTED_LOCALES.map((l) => fc.constant(l))),
          fc.array(fc.oneof(...SUPPORTED_LOCALES.map((l) => fc.constant(l))), {
            minLength: 1,
            maxLength: 7,
          }),
