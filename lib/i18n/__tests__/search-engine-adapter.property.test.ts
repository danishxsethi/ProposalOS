    ...
    fc.assert(
      fc.property(
        ...
        (locale, [typeA, typeB]) => {
          const adapter = new SearchEngineAdapter();
          const engine = adapter.getPrimarySearchEngine(locale);
          const guidanceA = adapter.generateSearchEngineGuidance(
            { type: typeA, description: 'desc' },
            locale
          );
          const guidanceB = adapter.generateSearchEngineGuidance(
            { type: typeB, description: 'desc' },
            locale
          );
