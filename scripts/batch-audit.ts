...
async function runPropose(
  auditId: string
): Promise<{ proposalId: string; webLinkToken: string; qaScore?: number }> {
...
function generateReport(results: AuditResult[]): string {
  ...
  const avgScore =
    withScore.length > 0
      ...
      : 'N/A';
  const successRate =
    results.length > 0 ? ((successful.length / results.length) * 100).toFixed(1) : '0';

  const byVertical = new Map<string, { success: number; failed: number; scores: number[] }>();
...
async function main(): Promise<void> {
  ...
  const processWithRateLimit = async (target: Target): Promise<AuditResult> => {
    ...
    console.log(
      `[${completed}/${targets.length}] ${icon} ${target.businessName} (${target.vertical}) — ${qa}`
    );
    return result;
  };
