...
import { transitionToClient } from '@/lib/crm/statusTransition';
...
export async function handlePaymentSuccess(
  ...
  if (lead.proposalId) {
    await prisma.proposal.update({
      where: { id: lead.proposalId },
      data: { status: 'ACCEPTED' as any, tierChosen: tier },
    });
  }
  ...
  // CRM transition: Lead -> Client (FIX-03)
  await transitionToClient(lead.id);

  // Transition to closed_won and set SLA clock (14 days)
  const slaDeadline = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000);
  await prisma.prospectLead.update({
    where: { id: leadId },
    data: {
      pipelineStatus: 'closed_won',
      metadata: { SLA_Deadline: slaDeadline.toISOString(), tenantProvisioned: true }
    },
  });

  // Trigger onboarding flow
  if (lead.decisionMakerEmail && tier) {
    await sendOnboardingEmail({
      proposalId: lead.proposalId || '',
      clientEmail: lead.decisionMakerEmail,
      clientName: lead.businessName || undefined,
      tier,
      tenantId: lead.tenantId,
    });
  }

  console.log(`[DealCloser] Tenant provisioned. SLA Tracking Clock initiated with deadline ${slaDeadline.toISOString()}`);

  // Initiate delivery could dispatch a queue job here
}
