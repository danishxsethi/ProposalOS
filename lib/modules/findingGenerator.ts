import { EffortLevel, FindingType } from '@prisma/client';

import type { ConversionDetectionResult } from '@/lib/analysis/conversionDetector';
import {
  type CompetitorGbpData,
  computeGbpCompleteness,
  type GbpCompletenessResult,
} from '@/lib/analysis/gbpCompleteness';
...
export function normalizeConfidence(
  raw: number,
  sourceScale: '0-100' | '1-10' | 'pagespeed'
): number {
...
function generateSchemaFindingsFromAnalysis(schema: SchemaAnalysis, pointer: string): Finding[] {
  ...
  for (const check of checks) {
    if (!check.present && check.recommendation) {
      const example =
        check.id === 'local-business-organization'
...
export function generateWebsiteFindings(data: any): Finding[] {
  ...
  if (a11yScore < 90) {
    findings.push({
      ...
      description:
        'Your site may be difficult to use for people with disabilities, limiting your audience.',
      evidence: [
        createEvidence({
          pointer: finalUrl || PSI_POINTER,
          source: 'pagespeed_v5',
...
function generateConversionFindingsFromAnalysis(
  ...
  if (conversion.criticalMissing.includes('Click-to-call phone number')) {
    findings.push({
      ...
      description:
        "Mobile visitors can't contact you with one tap — that's your highest-intent traffic. Add a tel: link in your header or hero.",
      evidence: [
        createEvidence({
          pointer,
          source: 'conversion_detector',
      ...
      effortEstimate: 'LOW',
      recommendedFix: [
        'Add <a href="tel:+1234567890">Call Now</a> in header/hero',
...
export function generateConversionFindings(
  ...
  if (
    missing.includes('Click-to-call phone link') ||
    (!elements.phone.clickToCall && elements.phone.present)
  ) {
    findings.push({
      module: 'conversion',
      category: 'conversion',
      ...
      description:
        "Without a click-to-call button, mobile visitors (60% of traffic) can't easily reach you. Wrap your phone number in a tel: link.",
      evidence: [
        createEvidence({
          pointer,
          source: 'conversion_module',
  ...
  if (missing.includes('CTA above the fold') && elements.ctas.count > 0) {
    findings.push({
      ...
      description:
        'Visitors need a clear next step before they scroll. Move your primary CTA (Book Now, Get Quote, Call) above the fold.',
      evidence: [
        createEvidence({
          pointer,
          source: 'conversion_module',
      ...
      effortEstimate: 'LOW',
      recommendedFix: [
        'Move primary CTA to hero/header',
        'Use contrasting button color for visibility',
      ],
    });
  }
  ...
  if ((data as any).content) {
    ...
    if (keywords && keywords.length > 0) {
      ...
      findings.push({
        ...
        type: 'VITAMIN',
        title: isStuffing
          ? `Possible keyword stuffing: "${topKeyword.word}"`
          : `Top keyword: "${topKeyword.word}"`,
        ...
        confidenceScore: normalizeConfidence(90, '0-100'),
        effortEstimate: 'LOW',
        recommendedFix: isStuffing
          ? ['Use synonyms instead of repeating the same keyword', 'Focus on natural language']
          : [],
      });
    }
  }
...
export function generateSchemaMarkupFindings(
  ...
  if (score < 70) {
    const primaryRec =
      recommendations[0] ??
      'Add structured data (JSON-LD) to help Google understand your business and show rich results.';
    findings.push({
      module: 'schemaMarkup',
      category: 'visibility',
    ...
    findings.push({
      ...
      evidence: [
        createEvidence({
          ...
          type: 'text',
          value: 'Missing',
          label: 'LocalBusiness/Organization',
        }),
      ],
  ...
  if (schemasMissing.includes('WebSite')) {
    findings.push({
      ...
      description:
        'Add WebSite schema with your site URL and name for sitelinks and search features in Google.',
      evidence: [
        createEvidence({
          pointer,
          source: 'schema_markup',
      ...
      confidenceScore: normalizeConfidence(95, '0-100'),
      effortEstimate: 'LOW',
      recommendedFix: ['Add WebSite JSON-LD with @context, @type WebSite, name, and url.'],
    });
  }
  ...
  const incompleteSchemas = schemasFound.filter(
    (s: { completeness?: number; missingProperties?: string[] }) =>
      (s.completeness ?? 100) < 100 && (s.missingProperties?.length ?? 0) > 0
  );
  for (const s of incompleteSchemas.slice(0, 2)) {
    ...
    if (schema.missingProperties?.length > 0) {
      findings.push({
        ...
        confidenceScore: normalizeConfidence(9, '1-10'),
        effortEstimate: 'LOW',
        recommendedFix: schema.missingProperties.map(
          (p) => `Add "${p}" property to your ${schema.type} schema.`
        ),
      });
    }
  }
...
export function generateAccessibilityFindings(
  ...
  const headingData = issuesByCategory?.headings as
    | { h1Count?: number; skipLevels?: boolean }
    | undefined;
  if (headingData) {
  ...
  const contrastData = issuesByCategory?.contrast as
    | { failCount?: number; worstRatio?: number }
    | undefined;
  if (contrastData && (contrastData.failCount ?? 0) > 0) {
    ...
    findings.push({
      ...
      effortEstimate: 'MEDIUM',
      recommendedFix: [
        'Aim for 4.5:1 contrast (normal text)',
        'Aim for 3:1 (large text)',
        'Use a contrast checker tool',
      ],
    });
  }

  const formData = issuesByCategory?.forms as
    | { percentage?: number; totalInputs?: number }
    | undefined;
  ...
  const linkData = issuesByCategory?.links as
    | { genericCount?: number; examples?: string[] }
    | undefined;
  if (linkData && (linkData.genericCount ?? 0) > 0) {
    ...
    findings.push({
      ...
      effortEstimate: 'LOW',
      recommendedFix: [
        'Replace "click here" with "Download the report"',
        'Replace "read more" with the article title',
      ],
    });
  }
  ...
  if (recommendations.some((r) => r.includes('lang='))) {
    findings.push({
      ...
      description:
        'Add lang="en" (or appropriate language) to help screen readers and search engines.',
      evidence: [
        createEvidence({
          pointer,
          source: 'accessibility_scan',
  ...
  if (recommendations.some((r) => r.includes('viewport'))) {
    findings.push({
      ...
      description:
        'Required for mobile accessibility. Without it, pages may not scale properly on small screens.',
      evidence: [
        createEvidence({
          pointer,
          source: 'accessibility_scan',
...
export function generateSecurityFindings(
  ...
  if (httpsData && !httpsData.enabled) {
    findings.push({
      ...
      description:
        "Your visitors see a 'Not Secure' warning in their browser. Customers won't enter payment or personal info on an unencrypted site.",
      evidence: [
        createEvidence({
          pointer,
          source: 'security_audit',
  ...
  if (httpsData?.certificate && !httpsData.certificate.valid && httpsData.enabled) {
    findings.push({
      ...
      description:
        'Browsers will show a security warning. Visitors may leave before entering your site.',
      evidence: [
        createEvidence({
          pointer,
          source: 'security_audit',
  ...
  const missingHeaders =
    (data as SecurityResult['data']).headers?.filter((h) => h.status === 'missing') ?? [];
  if (missingHeaders.length >= 3 && score < 85) {
    findings.push({
      ...
      description:
        "Enterprise-grade security headers protect your customers' data. Missing headers can expose your business to data breaches and legal liability.",
      evidence: [
        createEvidence({
          pointer,
          source: 'security_audit',
  ...
  if (mixedContent) {
    findings.push({
      ...
      description:
        'Some resources load over HTTP on your HTTPS page. Browsers may block them, breaking your site. Fix for a seamless experience.',
      evidence: [
        createEvidence({
          pointer,
          source: 'security_audit',
 ...
 */
function computeCompetitorCompletenessScore(comp: {
  photosCount?: number;
  reviewCount?: number;
  rating?: number;
  hasHours?: boolean;
...
export function generateGBPFindings(
  ...
  const gbpPointer =
    website ||
    (placeId ? `https://www.google.com/maps/place/?q=place_id:${placeId}` : PLACES_POINTER);
  ...
  if (!website) {
    findings.push({
      ...
      description:
        "Customers searching for you on Google can't find your website, losing you potential business.",
      evidence: [
        createEvidence({
          pointer: gbpPointer,
          source: 'places_api_v1',
  ...
  if (!openingHours) {
    findings.push({
      ...
      description:
        "Customers can't see when you're open, which may cause them to choose a competitor instead.",
      evidence: [
        createEvidence({
          pointer: gbpPointer,
          source: 'places_api_v1',
  ...
  const competitorScores = competitorData?.topCompetitors
    ...
    .filter((s) => s > 0);
  const competitorGbpData: CompetitorGbpData[] | undefined = competitorData?.topCompetitors?.map(
    (c: any) => ({
      name: c.name,
...
export function generateCompetitorFindings(data: any, businessName: string): Finding[] {
  ...
  const serpPointer = serpQuery
    ? `https://serpapi.com/search?q=${encodeURIComponent(serpQuery)}`
    : SERPAPI_POINTER;
  ...
  if (!topCompetitors || topCompetitors.length === 0) {
    ...
    findings.push({
      ...
      description:
        "You don't show up when people search for your services in your area. Competitors are getting all the clicks.",
      evidence: [
        createEvidence({
          pointer: serpPointer,
          source: 'serpapi_v1',
          ...
          value: 'not_found',
          label: 'Local Pack',
          raw: { keyword, location, found: false },
        }),
      ],
  ...
  if (comparisonMatrix && comparisonMatrix.gaps) {
      ...
      findings.push({
        ...
        evidence: [
          createEvidence({
            ...
            raw: {
              gap: reviewGap.gap,
              competitorAvg: reviewGap.competitorAvg,
      ...
      findings.push({
        ...
        evidence: [
          createEvidence({
            ...
            raw: {
              gap: ratingGap.gap,
              competitorAvg: ratingGap.competitorAvg,
              matrix: comparisonMatrix,
            },
          }),
        ],
    ...
    if (speedGap && speedGap.gap < -10) {
      // arbitrary threshold, if we are >10 points slower
      findings.push({
        module: 'competitor',
        category: 'performance',
        type: 'VITAMIN',
        ...
        evidence: [
          createEvidence({
            ...
            value: speedGap.gap,
            label: 'Speed Gap',
            raw: { gap: speedGap.gap, competitorAvg: speedGap.competitorAvg },
          }),
        ],
        ...
        confidenceScore: normalizeConfidence(8, '1-10'),
        effortEstimate: 'MEDIUM',
        recommendedFix: ['Optimize images', 'Minify code', 'Use better hosting'],
      });
    }
      ...
      findings.push({
        ...
        evidence: [
          createEvidence({
            ...
            value: photoGap.gap,
            label: 'Photo Gap',
            raw: { gap: photoGap.gap, competitorAvg: photoGap.competitorAvg },
          }),
        ],
        ...
        confidenceScore: normalizeConfidence(7, '1-10'),
        effortEstimate: 'LOW',
        recommendedFix: ['Upload high-quality team and project photos', 'Add photos weekly'],
      });
    }
    ...
    const avgCompetitorReviews =
      topCompetitors.reduce((sum: number, c: any) => sum + (c.reviews || 0), 0) /
    ...
    findings.push({
      ...
      description:
        'Your competitors have more social proof. To compete, you need to close the review gap.',
      evidence: topCompetitors.slice(0, 3).map((c: any) =>
        createEvidence({
...
export function generateReputationFindings(data: any): Finding[] {
  ...
  const { negativeRatio, responseRate, avgRating, reviewCount, commonThemes, oldestReviewMonths } =
    summary;
  ...
  if (negativeRatio > 0.3) {
    findings.push({
      ...
      description:
        negativeThemesSummary ||
        'A high percentage of negative reviews is actively hurting your business reputation and driving away potential customers.',
      evidence: reviews
  ...
  if (responseRate === 0 && reviewCount > 0) {
    findings.push({
      ...
      description:
        'Responding to reviews shows you care about customer feedback. Unanswered reviews make your business look unresponsive.',
      evidence: [
        createEvidence({
          pointer: GOOGLE_REVIEWS_POINTER,
          source: 'google_reviews',
          ...
          value: 0,
          label: 'Response Rate',
          raw: { responseRate: 0, reviewCount },
        }),
      ],
  ...
  if (negativeReviews.length >= 3 && negativeThemesSummary) {
    findings.push({
      ...
      description:
        'Multiple customers are mentioning similar issues. This indicates a systemic problem that needs to be addressed.',
  ...
  if (oldestReviewMonths >= 6 && reviewCount <= 5) {
    findings.push({
      ...
      description:
        "Fresh reviews signal an active business. Stale review profiles may make potential customers question if you're still operating.",
      evidence: [
        createEvidence({
          pointer: GOOGLE_REVIEWS_POINTER,
          source: 'google_reviews',
          type: 'metric',
          value: oldestReviewMonths,
          label: 'Oldest Review (months)',
          raw: { oldestReviewMonths, reviewCount },
        }),
      ],
  ...
  const negativeResponseRate =
    negativeReviews.length > 0 ? negativeWithResponse / negativeReviews.length : 1;
  if (negativeReviews.length >= 2 && negativeResponseRate < 0.5) {
    findings.push({
      ...
      description:
        'Responding to negative reviews is critical for reputation recovery. It shows potential customers you take feedback seriously.',
      evidence: negativeReviews
        .filter((r: any) => !r.hasOwnerResponse)
        .slice(0, 2)
        .map((r: any) =>
          createEvidence({
            pointer: GOOGLE_REVIEWS_POINTER,
...
export function generateSocialFindings(data: any): Finding[] {
  ...
  if (totalCount === 0) {
    findings.push({
      ...
      description:
        'Your website has no links to social media profiles. Customers expect to find you on social platforms to see reviews, photos, and updates.',
      evidence: [
        createEvidence({
          pointer: socialPointer,
          source: 'website_html',
          type: 'metric',
          value: 0,
          label: 'Platforms Found',
          raw: { platformsFound: 0 },
        }),
      ],
    ...
    findings.push({
      ...
      evidence: [
        createEvidence({
          ...
          value: totalCount,
          label: 'Platforms',
          raw: { platformsFound, platformsMissing },
        }),
      ],
      metrics: {
  ...
  if (totalCount > 0 && totalCount < 3) {
    findings.push({
      ...
      description:
        'Most successful local businesses maintain 3-4 active social profiles (Facebook, Instagram, LinkedIn, Google).',
      evidence: [
        createEvidence({
          pointer: socialPointer,
          source: 'website_html',
  ...
  if (totalCount >= 3) {
    findings.push({
      ...
      description:
        'Good social presence foundation. Make sure these profiles are active with regular posts and engagement.',
      evidence: [
        createEvidence({
          pointer: socialPointer,
          source: 'website_html',
