...
import { sendDeliveryHandoverEmail } from '@/lib/email/deliveryHandover';
...
async function generate_artifact(state: typeof DeliveryState.State) {
  const artifacts: GeneratedArtifact[] = [...state.artifacts];

  for (const finding of state.findings) {
    try {
      const existingIdx = artifacts.findIndex(a => a.id === finding.id);
      if (existingIdx !== -1 && artifacts[existingIdx].status === 'validated') {
        continue;
      }
  ...
  return { artifacts, retryCount: state.retryCount + (state.artifacts.length > 0 ? 1 : 0) };
}
...
async function upload_bundle(state: typeof DeliveryState.State) {
  ...
  try {
    ...
    // Send the delivery handover email to the client
    try {
      // Find client lead associated with this proposal
      const lead = await prisma.prospectLead.findFirst({
        where: { proposalId: state.proposalId }
      });

      if (lead?.decisionMakerEmail) {
        await sendDeliveryHandoverEmail({
          clientEmail: lead.decisionMakerEmail,
          businessName: lead.businessName,
          tenantId: state.tenantId,
          proposalId: state.proposalId,
          deliveryMessage: state.bundle.readmeContent.split('\n')[0] || 'Your implementation is ready.',
          gcsZipRef: zipUrl, // Use the generated URL directly or pass the GCS object reference
        });
        console.log(`Sent delivery handover email to ${lead.decisionMakerEmail}`);
      } else {
        console.warn(`Could not send delivery email: No client email found for proposal ${state.proposalId}`);
      }
    } catch (emailError) {
      console.error('Failed to send delivery handover email:', emailError);
      // We don't fail the node, we just log it
    }
...
function should_retry(state: typeof DeliveryState.State) {
  if (state.validationSummary.failedCount > 0 && state.retryCount < 3) {
    console.log(`[DeliveryGraph] Artifact validation failed. Retrying (Attempt ${state.retryCount + 1}/3)...`);
...
export const deliveryGraph = new StateGraph(DeliveryState)
  ...
  .addConditionalEdges("validate_artifact", should_retry)
