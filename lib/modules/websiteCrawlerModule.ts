...
function generateFindingsFromCrawl(crawlResult: CrawlResult, businessUrl: string): Finding[] {
  ...
  const missingTitlesPct =
    crawlResult.crawledPages.length > 0
  ...
  if (crawlResult.duplicateTitles.size > 0) {
    const duplicateCount = Array.from(crawlResult.duplicateTitles.values()).reduce(
  ...
  const avgPageSizeMB =
    crawlResult.crawledPages.length > 0
      ? crawlResult.crawledPages.reduce((sum, p) => sum + p.pageSizeKB, 0) /
        crawlResult.crawledPages.length /
        1024
      : 0;
