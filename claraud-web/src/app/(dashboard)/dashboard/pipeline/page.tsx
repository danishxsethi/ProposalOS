      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 h-[calc(100vh-200px)]">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="min-w-[300px] w-[300px] flex-shrink-0 bg-white/5 rounded-xl border border-white/10 p-4"
            >
    ...
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex-1 flex gap-4 overflow-x-auto pb-4 snap-x">
        {PIPELINE_STAGES.map((stage) => {
          ...
          const stageProposals =
            proposals?.filter(
              (p: any) => p.status === stage.id || (stage.id === 'LOST' && p.status === 'DECLINED')
            ) || [];
          ...
          return (
            <div
              key={stage.id}
              className="min-w-[320px] w-[320px] max-w-[320px] flex-shrink-0 flex flex-col snap-start"
            >
              <div
                className={`rounded-xl border ${stage.color} bg-bg-secondary h-full flex flex-col overflow-hidden`}
              >
