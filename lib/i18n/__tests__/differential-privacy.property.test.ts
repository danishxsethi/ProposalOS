...
const epsilonArb = fc
  .double({ min: 0.01, max: 10.0, noNaN: true })
  .filter((e) => e > 0 && isFinite(e));

/** Valid sensitivity values */
const sensitivityArb = fc
  .double({ min: 0.1, max: 10.0, noNaN: true })
...
const businessSizeArb = fc.oneof(fc.constant('small'), fc.constant('medium'), fc.constant('large'));
...
describe('Property 32: Differential Privacy Application', () => {
    ...
    fc.assert(
      fc.property(anonymizedMetricsArb, epsilonArb, epsilonArb, (data, eps1, eps2) => {
        ...
        for (let i = 0; i < runs; i++) {
          totalNoiseSmall += engine.applyDifferentialPrivacy(
            data,
            smallEps
          ).differentialPrivacyNoise;
          totalNoiseLarge += engine.applyDifferentialPrivacy(
            data,
            largeEps
          ).differentialPrivacyNoise;
        }
  ...
  it('should apply DP with any valid epsilon and sensitivity combination', () => {
    fc.assert(
      fc.property(
        anonymizedMetricsArb,
        epsilonArb,
        sensitivityArb,
        (data, epsilon, sensitivity) => {
