    ...
    addApiCall(api: ApiType, count: number = 1) {
        ...
        if (this.totalCents > 100 && !this.usage['ALERT_HUNDRED_CENTS_EMITTED']) {
            logger.warn({
                ...
                usage: this.usage
            }, 'Cost Tracker Alert: Single audit exceeded $1.00 soft cap threshold.');
            this.usage['ALERT_HUNDRED_CENTS_EMITTED'] = 1;
        }

        if (this.totalCents > 200) {
            logger.error({
                totalCents: this.totalCents,
                usage: this.usage
            }, 'HARD CAP ALERT: Single audit exceeded $2.00 strict limit.');
            throw new Error('CostTracker hard cap exceeded: Audit terminated to prevent cost overrun.');
        }
    }
