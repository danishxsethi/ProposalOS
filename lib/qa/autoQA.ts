...
export function runAutoQA(
  ...
  const truthChecks: QAResult[] = [
    ...
    },
    {
      ...
      details: impactScoreRangeValid
        ? 'All scores valid (1-10)'
        : 'Some impact scores out of range',
    },
    {
      category: 'Truth',
      check: 'No Duplicates',
  ...
  const decisionChecks: QAResult[] = [
    ...
    },
    {
      ...
      details: hasRoiScenarios
        ? 'All tiers include valid ROI scenarios'
        : 'One or more tiers missing ROI scenario block',
    },
    {
      category: 'Decision',
    ...
    },
    {
      ...
      passed: clearCta,
      details: clearCta
        ? 'At least one CTA detected in next steps'
        : 'No clear CTA found in next steps',
    },
  ];
  ...
  if (requiresHumanReview)
    warnings.push('Top proposal requires 2-minute human closeability review');
