...
export async function onboardPartner(config: PartnerConfig): Promise<string> {
  return withSystemDbBypass('pipeline:partner-portal:onboard-partner', async (prisma) => {
...
export async function matchLeadsToPartner(partnerId: string): Promise<PackagedLead[]> {
  return withSystemDbBypass('pipeline:partner-portal:match-leads', async (prisma) => {
    ...
    // Find prospects matching partner's verticals and geographies (across all tenants —
    // partner matching is an inherently cross-tenant lead-marketplace operation).
...
export async function deliverLead(partnerId: string, leadId: string): Promise<PackagedLead> {
  return withSystemDbBypass('pipeline:partner-portal:deliver-lead', async (prisma) => {
    const partner = await prisma.agencyPartner.findUnique({
      where: { id: partnerId },
    });
    ...
    await prisma.partnerDeliveredLead.create({
      data: {
        ...
        packagedData: packagedData as any,
        status: 'delivered',
      },
    });
...
export async function updateLeadStatus(
  ...
  await withSystemDbBypass('pipeline:partner-portal:update-lead-status', (prisma) =>
    prisma.partnerDeliveredLead.updateMany({
      where: {
        partnerId,
        leadId,
      },
...
export async function getPartnerMetrics(partnerId: string): Promise<PartnerMetrics> {
  return withSystemDbBypass('pipeline:partner-portal:get-metrics', async (prisma) => {
    const partner = await prisma.agencyPartner.findUnique({
      where: { id: partnerId },
    });
