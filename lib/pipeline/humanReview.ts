...
export async function routeToReview(
  ...
  await transition(prospectId, 'hot_lead', 'deal_closer' as any);
...
export async function getReviewQueue(
  ...
  // Fetch audits and proposals manually
  const items: ReviewQueueItem[] = await Promise.all(prospects.map(async prospect => {
    const painBreakdown = (prospect.painBreakdown as Record<string, number>) || {};
    const painScore = Object.values(painBreakdown).reduce((sum: number, val: number) => sum + val, 0);

    const audit = prospect.website ? await prisma.audit.findFirst({ where: { businessUrl: prospect.website } }) : null;
    const proposal = audit ? await prisma.proposal.findFirst({ where: { auditId: audit.id } }) : null;
...
export async function approveProspect(action: ReviewAction): Promise<void> {
  ...
  await transition(prospectId, 'closing', 'human_review' as any);
...
export async function rejectProspect(action: ReviewAction): Promise<void> {
  ...
  await transition(prospectId, 'closed_lost', 'human_review' as any);
...
export async function getProspectContext(prospectId: string): Promise<ReviewQueueItem | null> {
  ...
  const audit = prospect.website ? await prisma.audit.findFirst({
    where: { businessUrl: prospect.website },
...
export async function overrideProspectStatus(
  ...
  await transition(prospectId, newStatus as any, 'manual_override' as any);
