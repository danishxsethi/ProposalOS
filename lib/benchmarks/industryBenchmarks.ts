...
export const INDUSTRY_BENCHMARKS: Record<Industry, IndustryBenchmarks> = {
  ...
  default: {
    // General local business average
...
export function compareToBenchmark(
  ...
  const industry = (
    INDUSTRY_BENCHMARKS[industryKey as Industry] ? industryKey : 'default'
  ) as Industry;
...
// 3. Finding Enhancer
export function threadBenchmarksThroughFindings(findings: Finding[], industry: string): Finding[] {
...
export function generateBenchmarkRadarData(findings: Finding[], industryKey: string) {
  const industry = (
    INDUSTRY_BENCHMARKS[industryKey as Industry] ? industryKey : 'default'
  ) as Industry;
