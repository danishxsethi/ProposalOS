...
async function main() {
  ...
  try {
    ...
    console.log(
      `   Before: Impact=${findingToEdit.impactScore}, Excluded=${findingToEdit.excluded}`
    );
    ...
    console.log(
      `   After: Impact=${editedFinding.impactScore}, ManuallyEdited=${editedFinding.manuallyEdited}`
    );
    ...
    if (audit.findings.length > 1) {
      ...
      const deleteResponse = await fetch(
        `http://localhost:3000/api/finding/${findingToExclude.id}`,
        {
          method: 'DELETE',
        }
      );
    ...
    console.log(
      `   • Web link: http://localhost:3000/proposal/${regeneratedProposal.webLinkToken}`
    );
    ...
    console.log(
      `   • Executive summary length: ${newProposal.executiveSummary?.length || 0} chars`
    );
