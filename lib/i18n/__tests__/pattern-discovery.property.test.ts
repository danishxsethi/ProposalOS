  ...
  it('should return patterns for every unique (platform, plugin, issueType) combination in findings', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        industryArb,
        localeArb,
        async (findings, industry, locale) => {
          ...
          const uniqueKeys = new Set(
            findings.map((f) => {
              ...
              return plugin ? `${platform}::${plugin}::${issueType}` : `${platform}::${issueType}`;
            })
          );
  ...
  it('should make promoted patterns queryable via queryPatterns()', async () => {
    await fc.assert(
      fc.asyncProperty(findingArb, industryArb, localeArb, async (finding, industry, locale) => {
        ...
        expect(
          results.some((p) =>
            p.affectedPlatforms.some((pl) => pl.toLowerCase() === platform.toLowerCase())
          )
        ).toBe(true);
      }),
      { numRuns: 100 }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (plugin, industry, locale) => {
          ...
          for (const pattern of results) {
            expect(
              (pattern.affectedPlugins ?? []).some((p) => p.toLowerCase() === plugin.toLowerCase())
            ).toBe(true);
          }
        }
      ),
