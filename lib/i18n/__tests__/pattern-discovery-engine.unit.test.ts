...
describe('PatternDiscoveryEngine', () => {
  ...
  describe('queryPatterns()', () => {
    beforeEach(async () => {
      await engine.analyzeAudit(
        'a1',
        [makeFinding({ platform: 'WordPress', plugin: 'Yoast', issueType: 'faq-schema' })],
        'dental',
        'en-US'
      );
      await engine.analyzeAudit(
        'a2',
        [makeFinding({ platform: 'Shopify', plugin: undefined, issueType: 'og-tags' })],
        'ecommerce',
        'de-DE'
      );
      await engine.analyzeAudit(
        'a3',
        [makeFinding({ platform: 'WordPress', plugin: 'RankMath', issueType: 'sitemap' })],
        'medical',
        'fr-FR'
      );
    });
