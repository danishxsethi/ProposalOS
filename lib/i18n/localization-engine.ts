...
and create a culturally appropriate prompt.`;
  ...
  private async localizeFinding(findings: any[], locale: string): Promise<LocalizedFinding[]> {
    return Promise.all(
      findings.map(async (finding) => ({
        ...
        localizedText: await this.translateContent(
  ...
  private hasPrivacyActImplications(rec: LocalizedRecommendation): boolean {
    const text = (rec.localizedText || '').toLowerCase();
    return (
      text.includes('personal information') || text.includes('privacy') || text.includes('data')
    );
  }
  ...
  /**
   * Get RTL-aware CSS class name
   */
  getRTLClass(baseClass: string, locale: string): string {
    return isRTL(locale) ? `${baseClass}-rtl` : baseClass;
  }
