...
import type { AuditForEmail, ProposalForEmail, PlaybookForEmail, EmailSequence } from './types';
import type { QualityReport } from './qualityCheck';
...
export async function generateEmailSequence(
  ...
  const meta: ProspectMetadata = {
    industry: audit.businessIndustry || 'B2B',
    role: 'Decision Maker',
    sizeScope: 'SMB',
  };

  const executiveSummary = proposal.executiveSummary || 'New proposal for optimization.';
  const roiData = proposal.pricing ? JSON.stringify(proposal.pricing) : 'ROI improvements anticipated.';

  const result = await generateEmailSequenceNode(
    proposal.id,
    audit.findings.map(f => f.title).join(', '),
    executiveSummary,
    roiData,
    meta
  );
  ...
  return {
    emails: result.emails.map(e => ({
      ...
      subject: e.subjectA,
      body: e.body,
      previewText: e.subjectB,
      personalizationScore: 8,
    })),
    metadata: {
      businessName: audit.businessName,
      vertical: audit.businessIndustry || 'Unknown',
      topFinding: audit.findings[0]?.title || 'Optimization Opportunity',
...
export async function regenerateEmailsWithFeedback(
  ...
  // Simple fallback passing straight through for now
  return sequence;
}
