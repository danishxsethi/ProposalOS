...
function extractPageTexts(
  crawledPages: Array<{ url: string; html: string; title?: string }>
 ...
 * Calculate readability metrics
 */
function calculateReadabilityMetrics(
...
function generateContentFindings(
  ...
  const hasServicesPage = pageTexts.some(
    (p) => p.url.toLowerCase().includes('service') || p.title.toLowerCase().includes('service')
  );
  const servicesGap = analysis.contentGaps.some((gap) => gap.toLowerCase().includes('service'));
  ...
  if (!hasServicesPage || servicesGap) {
    findings.push({
      ...
      description:
        "Website lacks a dedicated services page or does not clearly list all services offered. Customers can't find what they need.",
      impactScore: 6,
  ...
  const hasAboutPage = pageTexts.some(
    (p) => p.url.toLowerCase().includes('about') || p.title.toLowerCase().includes('about')
  );
  ...
  if (!hasAboutPage) {
    findings.push({
      ...
      description:
        "Website lacks an About page. Customers want to know who they're working with — this builds trust and credibility.",
      impactScore: 4,
      confidenceScore: normalizeConfidence(90, '0-100'),
      evidence: [
        {
  ...
  const avgTrustScore =
    analysis.pages.reduce((sum, p) => sum + p.trustBuilding, 0) / analysis.pages.length;
