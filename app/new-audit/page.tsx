...
export default function NewAuditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const body = inputType === 'name' ? { name, city } : { url };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-8">
        ...
        <div className="flex gap-2 mb-6 bg-gray-900 p-1 rounded-lg">
          <button
            ...
            className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors ${
              inputType === 'name' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Business Name
          </button>
          <button
            ...
            className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors ${
              inputType === 'url' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Website URL
          </button>
        ...
        <p className="mt-6 text-xs text-gray-500 text-center">
          The audit will analyze website performance, Google Business Profile, and competitor
          comparison
        </p>
      </div>
    </div>
