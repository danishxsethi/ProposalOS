 ...
 */
export function calculateReportCard(findings: Finding[], industry: Industry): ReportCard {
...
function calculateCategoryScores(findings: Finding[]): CategoryScores {
  ...
  const hasClickToCall = findings.some(
    (f) => f.category === 'Conversion' && f.title.includes('Call')
  );
  const hasBooking = findings.some(
    (f) => f.category === 'Conversion' && f.title.includes('Booking')
  );
...
function generateGradingNarrative(score: number, grade: string, scores: CategoryScores): string {
  ...
  if (grade === 'A') {
    narrative +=
      'You are performing exceptionally well across the board, outpacing most competitors.';
  } else if (grade === 'B') {
    narrative +=
      'You have a solid foundation but are missing some key opportunities to dominate your market.';
  } else if (grade === 'C') {
    narrative +=
      "You are effectively average. You're invisible to many potential customers and losing leads to top competitors.";
  } else if (grade === 'D') {
    narrative +=
      'Your digital presence is actively hurting your business. You are visible but failing to convert, or simply not being found.';
  } else {
    narrative +=
      'Your digital presence is non-existent or critical broken. Immediate attention is required to stop leaking revenue.';
  }
