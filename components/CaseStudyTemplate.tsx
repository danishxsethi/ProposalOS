...
export default function CaseStudyTemplate({
  ...
  const painClusters =
    (proposal?.painClusters as Array<{ rootCause?: string; narrative?: string }>) ?? [];
  const challengeText =
    execSummary ||
    painClusters
      .map((c) => c.narrative || c.rootCause)
      .filter(Boolean)
      .join(' ') ||
  ...
  const baseUrl =
    typeof window !== 'undefined'
      ...
      : process.env.NEXT_PUBLIC_BASE_URL || 'https://proposalengine.com';
  const auditUrl = proposal?.webLinkToken
    ? `${baseUrl}/proposal/${proposal.webLinkToken}`
    : `${baseUrl}/audit`;
      ...
      <div className="case-study-document">
        ...
        <div className="case-study-page pdf-content case-study-cta-page">
          ...
          <p className="pdf-body">
            Get your free website audit. We&apos;ll analyse your digital presence and show you
            exactly where you&apos;re losing customers — and how to fix it.
          </p>
          <a href={auditUrl} className="case-study-cta-btn">
            Get your free audit
          </a>
          {branding.contact?.email && (
            <p className="case-study-contact">Questions? {branding.contact.email}</p>
          )}
        </div>
      </div>
    </div>
