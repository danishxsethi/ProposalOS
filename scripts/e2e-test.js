...
async function runTest(business, testNum) {
  ...
  try {
    ...
    console.log(
      `   ✓ Confidence: ${diagnoseData.diagnosis?.metadata?.clusteringConfidence || 'N/A'}`
    );
    ...
    console.log(
      `   ✓ Executive Summary: ${proposeData.proposal?.executiveSummary?.slice(0, 80)}...`
    );
    console.log(
      `   ✓ Pricing: $${proposeData.proposal?.pricing?.essentials}/$${proposeData.proposal?.pricing?.growth}/$${proposeData.proposal?.pricing?.premium}`
    );
    ...
    console.log(`\n✅ TEST ${testNum} PASSED`);
    console.log(
      `   Total Duration: ${results.audit.duration + results.diagnose.duration + results.propose.duration}ms`
    );
