      ...
      fc.assert(
        fc.property(signalArrayWithDuplicatesArb, (signals) => {
          ...
          for (const signal of deduplicated) {
            const windowKey = Math.floor(signal.detectedAt.getTime() / (24 * 60 * 60 * 1000));
      ...
      fc.assert(
        fc.property(
          ...
          (leadId, signalType, detectedAt, duplicateCount) => {
            ...
            const signals: DetectedSignal[] = Array.from(
      ...
      await fc.assert(
        fc.asyncProperty(
          detectedSignalArb.filter((signal) => {
            ...
            return (
              signal.detectedAt &&
              !isNaN(signal.detectedAt.getTime()) &&
