...
export class AnonymizationPipeline {
  ...
  private privacyConcernHandlers: Array<
    (errors: string[], data: AnonymizedAuditMetrics) => Promise<void>
  > = [];
  ...
  async anonymizeMetrics(rawMetrics: RawAuditMetrics): Promise<AnonymizedAuditMetrics> {
    ...
    const timestamp = rawTs instanceof Date && !isNaN(rawTs.getTime()) ? rawTs : new Date();
   ...
   */
  onPrivacyConcern(
    handler: (errors: string[], data: AnonymizedAuditMetrics) => Promise<void>
  ): void {
   ...
   */
  private applyGeneralization(field: string, value: any, rule: GeneralizationRule): any {
