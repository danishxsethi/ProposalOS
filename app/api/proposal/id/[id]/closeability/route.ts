...
function getGateWeightedScore(
  clientScoreResults: Record<string, unknown>,
  fallback: number
): number {
  ...
  return Math.round(
    truthScore * truthWeight + fitScore * fitWeight + decisionScore * decisionWeight
  );
}
...
export const POST = withAuth(async (req: Request, { params }: Params) => {
  try {
    ...
    const blendedScore =
      hardFails.length > 0 ? 0 : Math.round(baseScore * 0.85 + reviewerScore * 10 * 0.15);
