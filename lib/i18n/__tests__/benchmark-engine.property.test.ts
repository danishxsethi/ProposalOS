...
function makeMetrics(count: number, overrides: Partial<AnonymizedMetric> = {}): AnonymizedMetric[] {
  return Array.from({ length: count }, (_, i) => makeMetric({ value: 50 + i, ...overrides }));
}
...
const cohortMetricsBatchArb = fc
  ...
  .chain(([industry, businessSize, locale, metricType]) =>
    fc
      ...
      .map((values) =>
        values.map((value) => makeMetric({ industry, businessSize, locale, metricType, value }))
      )
...
describe('Property 13: Benchmark Metric Extraction', () => {
  ...
  it('should categorize stored metrics by industry, businessSize, and locale', async () => {
    await fc.assert(
      fc.asyncProperty(
        cohortMetricsBatchArb,
        async ({ metrics, industry, businessSize, locale }) => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ([industryA, industryB]) => {
          ...
          const cohortA = await engine.queryBenchmarks({
            industry: industryA,
            locale: 'en-US',
            businessSize: 'small',
          });
          const cohortB = await engine.queryBenchmarks({
            industry: industryB,
            locale: 'en-US',
            businessSize: 'small',
          });
...
describe('Property 15: Benchmark Cohort Fallback', () => {
  ...
  it('should fall back to industry+locale cohort when businessSize cohort is insufficient', async () => {
    await fc.assert(
      fc.asyncProperty(fc.tuple(industryArb, localeArb), async ([industry, locale]) => {
        const engine = new BenchmarkEngine();
...
describe('Property 35: Anonymized Metric Field Completeness', () => {
  ...
  it('should accept metrics with differentialPrivacyNoise field set', async () => {
    await fc.assert(
      fc.asyncProperty(fc.float({ min: 0, max: 5, noNaN: true }), async (noise) => {
        const engine = new BenchmarkEngine();
