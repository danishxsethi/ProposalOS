...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      'Could not fetch audits. Ensure server is running and you have completed audits with proposals.'
    );
  ...
  if (audits.length === 0) {
    console.log(
      'No audits with proposals found. Run some test audits first (e.g. npm run test:vertical-playbooks).'
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < Math.min(5, audits.length); i++) {
    ...
    try {
      ...
      const r = result as {
        emails?: Array<{ subject: string; score?: number; variant: number }>;
        bestVariant?: number;
      };
  ...
  const outPath = path.join(
    outDir,
    `cold_email_test_${new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)}.json`
  );
  fs.writeFileSync(
    outPath,
