...
/**
 * Generic upload to Google Cloud Storage
 * @returns Public URL of the uploaded file
 */
export async function uploadToGCS(buffer: Buffer, filename: string, contentType: string = 'application/octet-stream'): Promise<string | null> {
    if (!bucketName) {
        console.warn('GCS_BUCKET_NAME not set, skipping uploadToGCS');
        return null;
    }
