...
export async function POST(req: Request, { params }: { params: Promise<{ token: string }> }) {
    try {
        ...
        // Get the associated prospect lead for engagement tracking via website URL
        const prospectLead = proposal.audit?.businessUrl
            ? await prisma.prospectLead.findFirst({
                where: { website: proposal.audit.businessUrl, tenantId: proposal.tenantId! },
            })
            : null;
