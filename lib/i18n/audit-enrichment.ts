...
export class AuditEnrichment {
  ...
  async enrichAudit(
    ...
    const [benchmarkResponse, patternResponse, effectivenessResponse] = await Promise.allSettled([
      this.intelligenceAPI.queryBenchmarks({ industry, locale, size }, REQUESTER_ID),
      this.intelligenceAPI.queryPatterns({}, REQUESTER_ID),
      this.intelligenceAPI.queryEffectiveness({ industry }, REQUESTER_ID),
    ]);

    const benchmarks = benchmarkResponse.status === 'fulfilled' ? benchmarkResponse.value : null;
    const patterns =
      patternResponse.status === 'fulfilled' ? (patternResponse.value.data as any[]) : [];
    const effectiveness =
      effectivenessResponse.status === 'fulfilled' ? effectivenessResponse.value.data : null;
    ...
    const enrichedFindings = auditResults.findings.map((finding) =>
      this.enrichFinding(finding, benchmarks?.data ?? null, patterns, effectiveness)
    );
  ...
  enrichFinding(
    ...
    if (benchmarks != null && finding.metricType != null && finding.value != null) {
    ...
    if (effectiveness != null) {
      const effectivenessArray = Array.isArray(effectiveness) ? effectiveness : [];
    ...
    const hasAnyData = hasBenchmarkData || patternMatches.length > 0 || hasEffectivenessData;
    if (!hasAnyData) {
    ...
    for (const pattern of patterns) {
      ...
      if (
        ...
        pattern.affectedPlatforms.some(
          (p: string) => p.toLowerCase() === (finding.platform ?? "").toLowerCase()
        )
      ) {
        matched = true;
      }
      ...
      if (
        ...
        pattern.affectedPlugins.some(
          (p: string) => p.toLowerCase() === (finding.plugin ?? "").toLowerCase()
        )
      ) {
        matched = true;
      }
      ...
      if (!matched && pattern.description) {
        ...
        const patternWords = patternDescLower.split(/\s+/).filter((w: string) => w.length > 4);
        const overlap = patternWords.filter((w: string) => descriptionLower.includes(w));
        if (overlap.length >= 2) {
          matched = true;
        }
      }
