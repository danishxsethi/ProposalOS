  ...
  try {
    ...
    const hasIssues =
      ...
      input.crawlResult.crawledPages.some((p) => {
        const homepage = input.crawlResult?.crawledPages.find(
          (page) => page.url === input.businessUrl
        );
        return (
          homepage &&
          (homepage.h1Count === 0 ||
    ...
    if (hasIssues) {
      const homepageCrawl = input.crawlResult?.crawledPages.find(
        (p) => p.url === input.businessUrl
      );
    ...
    evidence.homepage.desktopAnnotated = results.find(
      (r) => r.name === 'homepage-desktop-annotated'
    );
    ...
    if (input.city) {
      const gbpResult = await captureGBPScreenshot(input.businessName, input.city, input.auditId);
      if (gbpResult) {
    ...
    if (
      ...
      for (
        let i = 0;
        i < Math.min(evidence.competitors.length, input.competitorUrls!.length, 2);
        i++
      ) {
 ...
 */
export function attachScreenshotsToFinding(finding: any, screenshots: ScreenshotResult[]): any {
