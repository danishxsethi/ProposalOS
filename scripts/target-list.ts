...
function addCmd(args: string[]): void {
  ...
  if (!name || !url || !vertical) {
    console.error(
      'Usage: add --name="..." --url="..." --vertical=... [--gbpUrl=...] [--address=...]'
    );
    process.exit(1);
  }
...
function updateCmd(args: string[]): void {
  ...
  if (isNaN(id)) {
    console.error(
      'Usage: update <id> [--name=...] [--url=...] [--gbpUrl=...] [--address=...] [--notes=...]'
    );
    process.exit(1);
  }
  const getArg = (key: string): string | undefined =>
...
function statsCmd(): void {
  ...
  const byVertical = new Map<
    string,
    { total: number; pending: number; success: number; error: number }
  >();
  ...
  for (const [v, s] of byVertical) {
    console.log(
      `| ${v.padEnd(12)} | ${String(s.total).padStart(5)} | ${String(s.pending).padStart(6)} | ${String(s.success).padStart(7)} | ${String(s.error).padStart(5)} |`
    );
  }
  ...
  console.log(
    `| ${'TOTAL'.padEnd(12)} | ${String(total).padStart(5)} | ${String(pending).padStart(6)} | ${String(success).padStart(7)} | ${String(error).padStart(5)} |`
  );
}
