...
export default function PresentationClient({ proposal, branding }: PresentationProps) {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isFullScreen, setIsFullScreen] = useState(false);
  const activeBranding = branding || BRANDING;
      ...
      render: () => (
        <div className="flex flex-col items-center justify-center h-full text-center space-y-8">
          ...
          <div className="mt-12 text-xl text-slate-500">
            Prepared by {activeBranding.name} • {new Date().toLocaleDateString()}
          </div>
        </div>
      ),
    },
    ...
    // ... Assuming we don't have a library handy, I'll skip complex chart and do category breakdown
    {
      ...
      render: () => (
        <div className="flex flex-col h-full justify-center px-12">
          <h2 className="text-4xl font-bold mb-16 text-center text-slate-200">
            Performance by Category
          </h2>
          <div className="grid grid-cols-3 gap-8">
            {['Performance', 'SEO', 'Accessibility', 'Security', 'Trust', 'Conversion'].map(
              (cat, i) => {
                ...
                return (
                  <div key={i} className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700">
                    ...
                    <div className="w-full bg-slate-700 h-4 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${score ?? 0}%` }} />
                    </div>
      ...
      render: () => (
        <div className="flex flex-col h-full px-12 py-12">
          <div className="flex items-center gap-4 mb-8">
            <span className="px-4 py-2 bg-red-500/20 text-red-400 rounded-lg text-xl font-bold">
              CRITICAL ISSUE #{i + 1}
            </span>
          ...
          <div className="grid grid-cols-2 gap-12 flex-1">
            <div className="flex flex-col justify-center space-y-8">
              <p className="text-2xl text-slate-300 leading-relaxed">{finding.description}</p>
              <div className="bg-slate-800/50 p-6 rounded-xl border-l-4 border-red-500">
                <h4 className="text-red-400 font-bold mb-2 uppercase tracking-wide">
                  Business Impact
                </h4>
                <p className="text-xl text-slate-200">
                  Impact Score:{' '}
                  <span className="text-red-400 font-bold">{finding.impactScore}/10</span>
                </p>
              </div>
        ...
        <div className="flex flex-col h-full px-12 py-12">
          ...
          <div className="grid grid-cols-3 gap-8 flex-1">
            {[
              ...
              <div
                key={i}
                className="bg-slate-800/40 border border-slate-700 rounded-2xl p-8 flex flex-col"
              >
                <div className="mb-6">
      ...
      render: () => (
        <div className="flex flex-col items-center justify-center h-full text-center space-y-12">
          ...
          <p className="text-slate-500 mt-8">
            Questions? Call us at {activeBranding.contact.phone}
          </p>
        </div>
      ),
    },
  ...
  // Track presentation slide changes
  useEffect(() => {
    const trackPresentationSlide = async () => {
      try {
        await fetch(`/api/proposal/token/${proposal.webLinkToken}/track`, {
          method: 'POST',
        ...
        console.error('Failed to track presentation slide:', error);
      }
    };

    trackPresentationSlide();
  }, [currentSlide, proposal.webLinkToken, sessionId, slides]);
    ...
    <div className="min-h-screen bg-[#0f172a] text-white overflow-hidden relative font-sans">
      ...
      <div className="absolute inset-0 flex flex-col">
        <div className="flex-1 relative">
          ...
          <div
            key={currentSlide}
            className="absolute inset-0 animate-fade-in px-8 sm:px-16 py-8 sm:py-12 flex flex-col"
          >
        ...
        <div className="absolute bottom-6 right-6 flex gap-4 text-slate-500 z-50">
          ...
          <a
            href={`/api/presentation/${proposal.webLinkToken}/export`}
            target="_blank"
            rel="noopener noreferrer"
            className="p-2 hover:text-white"
