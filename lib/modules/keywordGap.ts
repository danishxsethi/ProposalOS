...
function analyzeGaps(keywords: Keyword[], rankings: RankingCheck[]): KeywordGapAnalysis {
  ...
  const invisibleCount = rankings.filter(
    (r) =>
      r.rank === null &&
      !r.inLocalPack &&
...
function generateKeywordFindings(analysis: KeywordGapAnalysis, input: KeywordGapInput): Finding[] {
  ...
  const primaryGap = rankings.find(
    (r) =>
      r.rank === null &&
      !r.inLocalPack &&
  ...
  if (primaryGap) {
    findings.push({
      ...
      effortEstimate: 'HIGH',
      recommendedFix: [
        'Create a dedicated service page for this keyword',
        'Optimize GMB for this service',
      ],
    });
  }
  ...
  const nearMeGap = rankings.find(
    (r) => r.rank === null && !r.inLocalPack && r.keyword.includes('near me')
  );
  ...
  if (nearMeGap) {
    findings.push({
      ...
      recommendedFix: [
        'Ensure specific location/city is mentioned in titles',
  ...
  const longTailGaps = rankings.filter(
    (r) => r.rank === null && keywords.find((k) => k.term === r.keyword)?.category === 'long_tail'
  );
