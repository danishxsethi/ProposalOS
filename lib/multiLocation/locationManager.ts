...
export async function getGroupReport(groupId: string) {
  ...
  // Aggregate Stats
  const completedMembers = group.members.filter(
    (m: { audit: { status: string } }) => m.audit.status === 'COMPLETE'
  );
  ...
  // Identify Best/Worst
  const sorted = [...completedMembers].sort(
    (a, b) => (b.audit.overallScore || 0) - (a.audit.overallScore || 0)
  );
  const best = sorted[0];
  ...
  return {
    ...
    members: group.members.map(
      (m: {
        locationName: string;
        audit: { overallScore: number | null; status: string; id: string };
      }) => ({
        name: m.locationName,
