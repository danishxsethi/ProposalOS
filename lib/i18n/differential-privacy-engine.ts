...
export class DifferentialPrivacyEngine {
  ...
  applyDifferentialPrivacy(
    ...
    const avgNoise = data.metrics.size > 0 ? totalAbsoluteNoise / data.metrics.size : 0;
  ...
  private validateSensitivity(sensitivity: number): void {
    if (!isFinite(sensitivity) || sensitivity <= 0) {
      throw new RangeError(`Sensitivity must be a positive finite number, got ${sensitivity}.`);
    }
  }
}
