...
const nodeIdArb = fc.stringMatching(/^[a-z][a-z0-9-]{2,30}$/).filter((s) => s.length >= 3);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (nodeId, locales) => {
            ...
            const rows = uniqueLocales.map((locale) => makePromptRow({ node_id: nodeId, locale }));
            mockQuery.mockResolvedValueOnce({ rows });
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (nodeId, entries) => {
            ...
            expect(entry!.lastUpdated.getTime()).toBeGreaterThanOrEqual(entry!.createdAt.getTime());
          }
        ),
        { numRuns: 100 }
      );
