...
export async function validateApiKey(rawKey: string) {
  ...
  // The tenant is not known yet -- that's what this lookup determines -- so it must run
  // as an explicit system bypass rather than under any tenant's RLS scope.
  return withSystemBypass('api-key lookup by hash: tenant not yet known', async () => {
