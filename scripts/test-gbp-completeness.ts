...
import { type CompetitorGbpData, computeGbpCompleteness } from '@/lib/analysis/gbpCompleteness';
...
function roughCompetitorScore(c: {
  photosCount?: number;
  reviewCount?: number;
  rating?: number;
  hasHours?: boolean;
  ...
  for (const biz of TEST_BUSINESSES) {
    ...
    try {
      ...
      for (const b of completeness.breakdown) {
        ...
        console.log(
          `    ${icon} ${b.category}: ${b.score}/${b.maxScore} ${b.detail ? `(${b.detail})` : ''}`
        );
      }
